...
class Address_model extends CI_Model {
  ...
  PRIMARY KEY (id)
    ...
    function get_custom_address_type_users($addressBelongsTo){
      ...
      $db = $this->database;
      //$table = $this->get_table();
      $sql = "Select t.id, Concat(t.first_name, ' ', t.middle_name, ' ', t.surname) as fullname, t.profile_img, t.emp_code, t.contact_1, t.company_name  from ".$addressBelongsTo." t WHERE t.is_active=true and id not in (Select user_id from user_roles where account_type='".$addressBelongsTo."')";

      $check = $this->check_table($addressBelongsTo);
      if($check){
        $sql.=' UNION';
        $sql.=" Select concat(ur.login_id,'-','login') as id, Concat(t.first_name, ' ', t.middle_name, ' ', t.surname) as fullname, t.profile_img, t.emp_code, t.contact_1, t.company_name from user_roles ur inner join ".$addressBelongsTo." t on t.id=ur.user_id where ur.account_type='".$addressBelongsTo."' group by t.emp_code";
      }
      //echo $sql;
    	$query = $db->query($sql);
		//print_r($db->last_query());
