...
class Address extends MY_Controller {
	...
	function type_wise_user2($data = []){
		...
		$sql = 'Select a.id,a.site_name, a.pincode, a.tally_address, a.address_1, a.address_2 from customers c inner join address a on a.user_id=c.id and a.type="'.$userType.'" where a.user_id='.$userId.' and a.is_active=true';
		...
		foreach ($typeWiseUsers as $key => $typeWiseUser) {
			...
			$userList[$key+1]['text'] = (!empty($typeWiseUser['tally_address']))?$typeWiseUser['site_name']." - ".$typeWiseUser['tally_address']:$typeWiseUser['address_1']." ".$typeWiseUser['address_2']." ".$typeWiseUser['pincode'];
		}
