        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST') {
           ...
           if($this->form_validation->run()!== FALSE) { //echo "hii";
                ...
                if (empty($error)) {
                    $postData['primary_email'] = (empty(trim($postData['primary_email']))?NULL:trim($postData['primary_email']));
                    // echo '<pre>';print_r($postData);exit;
                    $reg_customer = json_decode($this->_register_admin_add_new($postData), true);
                    //print_r($reg_customer);exit;
...
<TALLYMESSAGE xmlns:UDF="TallyUDF">
 ...
 function importledger1()
    ...
    function export(){
        ...
        $tableColumns = ['Customer Code', 'Shop Name', 'Email Id', 'Contact 1', 'Referral Code', 'Entered By', 'Registered On', 'Status', 'Address', 'Area', 'Pincode'];
...
//aainternationalindia@gmail.com