...
class Orders extends MY_Controller {
    ...
    function export(){
        ...
        $tableColumns = ['Order Number', 'Shop Name', 'Contact Number', 'Order Date', 'Order Time', 'Address', 'Teritorry', 'Product specification', 'Sales Person', 'Entered By', 'Invoice Amount', 'Order Status'];
        $column = 0;
        ...
        foreach($query as $order){
            $orderDetails = $this->pktdblib->custom_query('Select od.*, p.product from order_details od inner join products p on p.id=od.product_id where od.order_id='.$order['id'].' and od.is_active=true');
            $str = '';
