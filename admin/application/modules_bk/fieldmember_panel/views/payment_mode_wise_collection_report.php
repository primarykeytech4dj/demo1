             ...
             <table id="report" class="table table-bordered table-striped report">
              <thead>
                <tr>
                  ...
                  <?php if(in_array(6, $this->session->userdata('roles'))){?>
                    <th class="text-center hideIt">Delivery Boy | Code</th>
                <?php }else{?>
                    <th class="text-center">Delivery Boy | Code</th>
                <?php } ?>
                  
                  <th class="text-center">Party Name | Order Code</th>
              ...
              <tbody>
               ...
               <tr>
                   <td><?=$dKey+1?></td>
                   <?php if(in_array(6, $this->session->userdata('roles'))){?>
                    <td class="hideIt"><?=$detail['delivery_person']?></td>
                <?php }else{?>
                    <td><?=$detail['delivery_person']?></td>
                <?php } ?>
                   
                   <td><?=ucfirst($detail['first_name'].' '.$detail['middle_name'].' '.$detail['surname']).' | '.$detail['order_code']?></td>
              ...
              <tfoot>
                  <tr>
                      <th colspan="<?=(in_array(6, $this->session->userdata('roles'))?2:3)?>">Total</th>
                      <?php foreach($paymentMode as $mode){ ?>
