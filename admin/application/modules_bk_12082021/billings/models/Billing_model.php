...
class Billing_model extends CI_Model {
   ...
   // Unique to models with multiple tables
	...
	{
		$table = $this->get_table();
		$this->db->select('concat(customers.first_name, " ", customers.middle_name, " ", customers.surname) as full_name, customers.company_name, '.$table.'.*, address.site_name');
		//$this->db->join('customer_sites', 'customer_sites.id='.$table.'.customer_site_id');
	...
	{
		$table = $this->get_table();
		$this->db->select('concat(customers.first_name, " ", customers.middle_name, " ", customers.surname) as full_name,customers.gst_no, customers.company_name, customer_bills.*, bill_payments.customer_bill_id as payment_id, (select count(customer_bill_details.id) from customer_bill_details where customer_bill_details.customer_bill_id = customer_bills.id) as annexure_count');
		$this->db->join('customers', 'customers.id=customer_bills.customer_id', 'left');
		$this->db->join('bill_payments', 'bill_payments.customer_bill_id=customer_bills.id','left');
