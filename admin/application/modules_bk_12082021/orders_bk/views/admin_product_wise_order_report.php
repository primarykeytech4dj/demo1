...
<div class="row">
  <div class="col-xs-12">
    <div class="box">
      ...
      <!-- /.box-header -->
      <div class="box-body">
          <?php echo form_open_multipart('orders/productreport', ['class'=>'form-horizontal', 'id' => 'stock_inward_report']);
    ...
    <?php } ?>
        ...
        <div class="col-md-12" id="dvData" style="overflow-x:auto">
          ...
          <table id="report" class="table table-bordered table-striped fixed-column-report">
            <thead>
              <tr>
                <th>Category</th>
                  ...
                  <tr>
                    ...
                    foreach($orders as $okey=>$order){
                        if(isset($orderDetails[$order['order_code']][$v['id']])){
                            $total=$total+$orderDetails[$order['order_code']][$v['id']]['qty'];
                            $orderCount = $orderCount+1;
                            $price = $price+ ($orderDetails[$order['order_code']][$v['id']]['qty_count']*$orderDetails[$order['order_code']][$v['id']]['price']);
                            //echo '<td>'.$orderDetails[$order['order_code']][$v['id']]['qty'].'</td>';
                        }else{
                            //echo '<td>0.00</td>';
                        }
                    }
                    $countArray[] = $orderCount;
                    $priceArray[] = $price;
                    echo '<td>'.$orderCount.'</td>';
                    echo '<td style="text-align:right">'.number_format($price, 2).'</td>';
                    echo '<td>'.$total." ".((count($uom)>1)?$uom[1]:'').'</td>';
                    echo '</tr>';
                    
                    
               }//end foreach
               echo '<tr><td colspan=2></td><td>'.array_sum($countArray).'</td><td style="text-align:right">'.number_format(array_sum($priceArray), 2).'</td><td></td></tr>';
