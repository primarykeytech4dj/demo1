...
class Address_model extends CI_Model {
  ...
  PRIMARY KEY (id)
    ...
    function get_custom_address_type_users($addressBelongsTo){
      ...
      if($check){
        $sql.=' UNION';
        $sql.=" Select concat(ur.login_id,'-','login') as id, Concat(t.first_name, ' ', t.middle_name, ' ', t.surname) as fullname, t.profile_img, t.emp_code from user_roles ur inner join ".$addressBelongsTo." t on t.id=ur.user_id where ur.account_type='".$addressBelongsTo."'";
      }
      //echo $sql;
    	$query = $db->query($sql);
		//print_r($db->last_query());
