			...
			<div class="nav-tabs-custom">
				<div class="tab-content">
					<div class="tab-pane active" id="basic_detail"> 
						<?php echo form_open_multipart('stocks/out2', ['class'=>'form-horizontal', 'id'=>'stockout_'.time()]); 
										...
										<div class="col-md-4">
											<div class="form-group">
												...
												<div class="col-sm-10">
													...
													<?=form_dropdown('data[stockout][order_code]', $option['orderCodes'], isset($values_posted['stockout']['order_code'])?$values_posted['stockout']['order_code']:'', ['class'=>'form-control select2 filter orderWiseDetail', 'data-link'=>"brokers/getorderCodeWisebrokers", 'data-target'=>'broker_id', 'id'=>'order_code'])?>
										...
										<div class="box-body" style="overflow-x:scroll">
											<table class="table">
												...
												if(isset($values_posted['stockout_details'])){
													if(count($values_posted['stockout_details'])>0){
														foreach ($values_posted['stockout_details'] as $key => $detail) { 
															/*echo '<pre>';print_r($detail);echo "</pre>";*/
														$productArray = ["id" =>'product_id_'.$key, 'required'=>'required', 'class'=>'form-control select2 filter stock', 'data-link' => 'stocks/getProductwiseDetail', "data-target" => 'stock_detail_id_'.$key, "style" => 'width:100% !important'];
														$coilArray = ["id" =>'stock_detail_id_<?=$key?>', 'required'=>'required', 'class'=>'form-control select2 stockDetail', "style" => 'width:100% !important'];
														$warehouseArray =  ["id" =>'company_warehouse_id_'.$key, 'required'=>'required', 'class'=>'form-control select2', "style" => 'width:100% !important'];
														/*if($flag==1){
															$productArray['disabled'] = 'disabled';
															$coilArray['disabled'] = 'disabled';
															$warehouseArray['disabled'] = 'disabled';
														}*/ ?>
													<tr id="<?=$key?>">
														<?php //print_r($option['product']);?>
														<td><input type="checkbox" name="stockout_details[<?=$key?>][order_detail_id]" class="check_to_dispatch" id="check_to_dispatch_<?=$key?>" value="<?=$detail['id']?>"></td>
														<td><?php echo form_dropdown("stockout_details[".$key."][product_id]",$option['product'], $detail['product_id'],$productArray);?>
															
															
														</td>
														<td ><?php echo form_dropdown("stockout_details[".$key."][stock_detail_id]",$option['coil_no'][$detail['product_id']][str_replace(" ","_",$detail['grade'])],$detail['stock_detail_id'],$coilArray);?>
															 <input type="text" readonly="readonly" id="balance_qty_<?=$key?>" value="Bal : <?=$detail['balance_qty']?> MT" name="bal[<?=$key?>][bal]" class="form-control input">
														</td>
														<td>
															<?php echo form_dropdown("stockout_details[".$key."][company_warehouse_id]",$option['companyWarehouse'],$detail['company_warehouse_id'], $warehouseArray);?>
														</td>
														<td>
														...
														<td>
															<input type="text" name="stockout_details[<?=$key?>][qty]" class="form-control dispatchqty calculate" id="qty_<?=$key?>" value="<?=$detail['qty']?>" readonly="readonly"
															>
														</td>
