...
class Admin_Panel extends MY_Controller {
    ...
    function index() {
		...
		$report = ['delivery'=>['icon'=>'fa-truck', 'data'=>[]], 'orders'=>['icon'=>'fa-shopping-cart', 'data'=>[]]];
		...
		if($roles!='Sales'){
			...
			$sql6 = 'Select count(id) as order_count from orders where order_status_id=9 and modified like "'.$date.'%"';
			if($roles=='Delivery Boy'){
