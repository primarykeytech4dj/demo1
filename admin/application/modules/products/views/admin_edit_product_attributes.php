			...
			<table class="table" id="target">
				<thead>
					<tr>
						...
						<?php if(!$product['overall_stock_mgmt']){ ?>
							<th>In Stock Qty</th>
						<?php } ?>
						<?php if(NULL!==$this->session->userdata('application')['tally'] && $this->session->userdata('application')['tally_item_has_multiple_attributes']){
						echo '<th>Tally Name</th>';
						} ?>
						<th>Remark</th>
						<th>Is Default</th>
						<th>Is Active</th>
				...
				if(count($product_attributes)>0){
					...
					<tr id="<?php echo $key; ?>" data-id="<?=$id?>">
						...
						<?php if(NULL!==$this->session->userdata('application')['tally'] && $this->session->userdata('application')['tally_item_has_multiple_attributes']){
    						?>
    						<td><input type="text" id="tallyname_<?=$key?>"  class="form-control required" value="<?=$value['tally_name']?>" name="product_attributes[<?=$key?>][tally_name]"></td>
    						<?php
						} ?>
						<td>
