					...
					if(isset($product['attributes']) && count($product['attributes'])>0){
					    ...
					    foreach($product['attributes'] as $aKey=>$attribute){
					        ...
					        ?>
            					...
            					<?php if($aKey==0){ ?>
            				    <td rowspan="<?=count($product['attributes'])?>"><button class="button primary-btn updaterow" title="Update" id="updatebtn_<?=$productKey?>"><i class="fa fa-edit"></i></button></td>
            				    <?php } ?>
            				</tr>
					        <?php
					        ++$counter;
...
<script type="text/javascript">
    ...
    $(document).on('click', '.updaterow', function(e){
        ...
        var rowCount = $("#"+trId+" td:first-child").attr('rowspan');
        var trIndex = $("#"+trId).index();
        //alert(trIndex);
        //var i = 0;
        for(var i=0; i<rowCount; i++){
            //alert((parseInt(trIndex)+i));
            $("tr:nth-child("+(parseInt(trIndex)+i+1)+") *").filter(':input').each(function(){
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if(data.status=="success"){
                    /*for(var i=0; i<rowCount; i++){
                        //alert((parseInt(trIndex)+i));
                        $("tr:nth-child("+(parseInt(trIndex)+i+1)+")").each(function(){
                            $("tr:nth-child("+(parseInt(trIndex)+i+1)+")").remove();
                        })
