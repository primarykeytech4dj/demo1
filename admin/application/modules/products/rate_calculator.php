				...
				foreach ($products as $productKey => $product) { 
					...
					if(isset($product['attributes']) && count($product['attributes'])>0){
					    ...
					    <tr id="<?=$counter?>">
            					...
            					<td rowspan="<?=count($product['attributes'])?>"><?=$product['category_name']?></td>
					    ...
					    foreach($product['attributes'] as $aKey=>$attribute){
					        ...
					        ?>
					        <td class="base_price"><?=form_input(['name'=>'products['.$counter.'][base_price]', 'value'=>$product['base_price'], 'type'=>'text', 'id'=>'base_price_'.$counter.$aKey, 'class'=>'base_price_'.$counter2])?></td>
...
<script type="text/javascript">
    $(document).on('blur', '.margin', function(){
        ...
        console.log(trId+" "+tdId+" "+product_id+" "+margin+" "+uom+" "+base_price);
    ...
    $('input[class^="base_price_"]').on('keyup', function(){
        var val = $(this).val();
