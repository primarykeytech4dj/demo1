...
class Products extends MY_Controller {
	...
	function __construct() {
		...
		$this->setup = Modules::run('setup/loadModuleWiseSetup', ['module_name'=>$this->module_name]);
		//$setup = $this->setup();	
	}
	...
	function admin_edit($id) {
		if($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			if($this->form_validation->run('products')!== FALSE){
				...
				if(empty($error)) {
					...
					if(($_SESSION['application']['product_has_multiple_categories']) && NULL!==($this->input->post("data[product_product_categories][product_category_id][0]"))){
					    $chk = $this->pktdblib->custom_query('SHOW COLUMNS FROM products like "product_category_id"');
					    if($chk){
					        $postData['product_category_id'] = $this->input->post("data[product_product_categories][product_category_id][0]");
					    }
					}
					...
					{	
					    if(!$postData['is_pack']){
					        $packProduct = $this->pktdblib->custom_query('Delete from pack_products where basket_id='.$id);
					    }
		...
		$data['gstRates'] = Modules::run('products/admin_edit_gst_rates', $id);
	...
	function admin_edit_product() {
		...
		$this->load->view('products/admin_edit', $data);
	}
		...
		if($_SERVER['REQUEST_METHOD']=='POST') {
			...
			if($this->form_validation->run('products')!== false) {
				...
				if(!empty($_FILES['product_images']['name'])) {
					...
					//echo '<pre>';print_r($regProduct);exit;
					...
					//echo '<pre>';print_r($productImage);exit;
						if(!empty($productImage)) {
					 	...
					 	/*echo '<pre>';
					 	print_r($this->input->post('data')['product_gst_rates']);*/
					 	if(NULL!==$this->input->post('data')['product_gst_rates']){
					 	    $gstData = $this->input->post('data')['product_gst_rates'];
					 	    $gstData['created_by'] = $this->session->userdata('user_id'); 
					 	    $gstData['created'] = date('Y-m-d H:i:s');
					 	    $gstData['product_id'] = $regProduct['id'];
					 	    $gstData['effective_from'] = $this->pktlib->dmYtoYmd($gstData['effective_from']);
					 	    $this->pktdblib->set_table('product_gst_rates');
					 	    //print_r($gstData);
		                    $gst = $this->pktdblib->_insert($gstData);
		                    //print_r($gst);
					 	}
					 	//exit;
					 	...
					 	$msg = array('message'=>$regProduct['message'], 'class'=>'alert alert-danger' );
					 	$this->session->set_flashdata('message', $msg);
					 }				
			} else {
	...
	function register_products($data) {
		...
		$result = $this->pktdblib->_insert($insert_data);
		//print_r($result);exit;
		if($result['status'] == 'success'){
			...
			return json_encode(['message'=>'Products Addded Successfully', 'status'=>'success', 'id'=>$product['id'], 'products'=>$product]);
		}else{
			return json_encode(['message'=>$result['message']['message'], 'status'=>'error']);
		}
	}
	...
	function admin_add_product() {
	    $data['setup'] = $this->setup;
		$this->load->view('products/admin_add', $data);
	}
    ...
    function update_stock($companyId=1){
    	if($_SERVER['REQUEST_METHOD'] == 'POST'){
    		...
    		$counter = 0;
    		/*echo '<pre>';
    		print_r($this->input->post('data[products]'));
    		exit;*/
    		foreach ($this->input->post('data[products]') as $key => $product) {
    		    //print_r($product);
    		    //echo 'Select pa.*, concat(a.unit," ",a.uom) as attribute from product_attributes pa inner join attributes a on a.id=pa.attribute_id where pa.product_id='.$product['id'].' and pa.is_active=true<br>';
    		    $attributes = $this->pktdblib->custom_query('Select pa.*, concat(a.unit," ",a.uom) as attribute from product_attributes pa inner join attributes a on a.id=pa.attribute_id where pa.product_id='.$product['id'].' and pa.is_active=true');
    			if(count($attributes)>0){
    			    $_POST['data']['products'][$key]['description'] = '';
    			    foreach($attributes as $attribute){
        			    $requireUOM = explode(" ", $attribute['attribute']);
        			    //echo '<br>'; print_r($product);
                        $baseUOM = explode(" ", $product['base_uom']);
                        $unit = $this->pktlib->unit_convertion($requireUOM[1], $baseUOM[1]);
                        //echo $unit." <br>";
                        $price = ceil(($product['base_price']+$attribute['per_unit_margin'])*$requireUOM[0]*$unit);
                        $productAttribute[$counter]['id'] = $attribute['id'];
                        $productAttribute[$counter]['price'] = $price;
                        $productAttribute[$counter]['mrp'] = $price;
                        $productAttribute[$counter]['modified'] = date('Y-m-d H:i:s');
                        $productAttribute[$counter]['modified_by'] = $this->session->userdata('user_id');
                        
                        $_POST['data']['products'][$key]['description'] = $_POST['data']['products'][$key]['description']."<p>".$attribute['attribute']." Pack : ".($product['base_price']+$attribute['per_unit_margin'])." Per KG</p>";
                        ++$counter;
    			    }
    			...
    			if(isset($product['show_on_website'])){
    			    if($product['show_on_website']!='')
    			        $_POST['data']['products'][$key]['show_on_website'] = $product['show_on_website'];
    			        else
    			        $_POST['data']['products'][$key]['show_on_website'] = 1;
    			}
    			else
    				$_POST['data']['products'][$key]['show_on_website'] = 0;

    			if(isset($product['is_sale'])){
    			    if($product['is_sale']!='')
    			    $_POST['data']['products'][$key]['is_sale'] = $product['is_sale'];
    			    else
    				$_POST['data']['products'][$key]['is_sale'] = 1;
    			}
    			else
    				$_POST['data']['products'][$key]['is_sale'] = 0;

    		}
    		/*echo '<pre>';
    		print_r($_POST['data']['products']);
    		exit;*/
    		$insert = [];
    		$update = [];
    		...
    		if(!empty($productAttribute)){
    		    ...
    		    $this->pktdblib->update_multiple('id', $productAttribute);
    		}
    		
    		$msg = array('message'=>'Data Updated Successfully', 'class'=>'alert alert-success', 'status'=>'success' );
    		if($this->input->is_ajax_request()){
    		    echo json_encode($msg);
            	exit;
    		}
   ...
   function admin_edit_product_attributes($productId) {
    ...
    function productwise_attributes($data=[]){
        if($_SERVER['REQUEST_METHOD']=='POST' || !empty($data)){
            ...
            //print_r($productAttributes);exit;
            foreach($productAttributes as $key=>$attribute){
                $response[$key+1]['id'] = $attribute['id'];
                $response[$key+1]['text'] = $attribute['attribute'];
                if(!$attribute['is_active'])
                {
                    $response[$key+1]['disable'] = true;
                }
    ...
    function rate_calculator(){
        if($_SERVER['REQUEST_METHOD']=='POST'){
            ...
            if($upd){
                $msg = array('message'=>'Price updated Successfully', 'class'=>'alert alert-success', 'status'=>'success');
                if($this->input->is_ajax_request()){
        		    echo json_encode($msg);
                	exit;
        		}
                ...
                $msg = array('message'=>'Some Error Occurred', 'class'=>'alert alert-danger', 'status'=>'error');
                if($this->input->is_ajax_request()){
        		    echo json_encode($msg);
                	exit;
        		}
    ...
    function rate_calculator_form($productId = NULL){
        $sql = 'Select p.*, c.category_name from products p left join product_categories c on c.id=p.product_category_id where p.is_active=true';
        if(NULL!==$productId){
        ...
        foreach($products as $pKey=>$product){
            if($_SESSION['application']['product_has_multiple_categories']){
                $cat = $this->pktdblib->custom_query('select GROUP_CONCAT( DISTINCT pc.category_name SEPARATOR ",\n") as category_name  from product_product_categories ppc inner join product_categories pc on pc.id=ppc.product_category_id where ppc.product_id='.$product['id']);
                //print_r($cat);
                $products[$pKey]['category_name'] = $cat[0]['category_name'];
            }
    ...
    function get_gst_rate(){
        if($_SERVER['REQUEST_METHOD']=='POST'){
            ...
            if(NULL===$params['product_id']){
                echo json_encode(['message'=>'Invalid Access', 'status'=>'fail']);
                exit;
            }
            $gst = [];
            
            $itemwiseGst = $this->pktdblib->custom_query('select gst_rate as gst, hsn_sac_code as hsn_code from product_gst_rates where effective_till IS NULL AND product_id='.$params['product_id']);
            if(empty($itemwiseGst)){
                if(!$this->session->userdata('application')['product_has_multiple_categories']){
                    $itemwiseGst = $this->pktdblib->custom_query('select pa.gst, pa.hsn_code from products p inner join product_categories pa on p.product_category_id=pa.id where p.id='.$params['product_id']);
                    $gst = $itemwiseGst[0];
                }
            }else{
                $gst = $itemwiseGst[0];
            }
            echo json_encode(['status'=>'success', 'data'=>$gst]);
            exit;
        }else{
            echo json_encode(['message'=>'Invalid Access', 'status'=>'fail']);
            exit;
    ...
    function admin_edit_gst_rates($productId) {
 		if($_SERVER['REQUEST_METHOD'] == 'POST'){ 
 		    /*echo '<pre>';
 		    print_r($this->input->post());
 		    exit;*/
			...
			$data['values_posted'] = $this->input->post();
		    $this->form_validation->set_error_delimiters('<span class="text-danger">','</span>');
		    foreach($data['values_posted']['product_gst_rates'] as $key=>$gst){
		        $gst['effective_from'] = $this->pktlib->dmYtoYmd($gst['effective_from']);
		        if(!empty($gst['effective_till'])){
		            $gst['effective_till'] = $this->pktlib->dmYtoYmd($gst['effective_till']);
		            $gst['is_active'] = false;
		        }else{
		            $gst['is_active'] = true;
		            $gst['effective_till'] = NULL;
		        }
		        if(isset($gst['id']) && ($gst['id']!=0 || $gst['id']!="")){
		            $gst['modified'] = date('Y-m-d H:i:s');
		            $gst['modified_by'] = $this->session->userdata('user_id');
		            $update[] = $gst;
		        }else{
		            $gst['created_by'] = $this->session->userdata('user_id');
		            $gst['created'] = date('Y-m-d H:i:s');
		            $insert[] = $gst;
		        }
		    }
				...
				$this->pktdblib->set_table('product_gst_rates');
				if(!empty($insert)){
					$flag = $this->pktdblib->_insert($insert);
				}

				if(!empty($update)){
					$flag = $this->pktdblib->update_multiple('id', $update);
				}
                /*echo $this->db->last_query();
                exit;*/
				if($flag){
					$msg = array('message'=>'GST updated Successfully', 'class'=>'alert alert-success' );
					$this->session->set_flashdata('message', $msg);
					redirect('products/editproduct/'.$productId.'?tab=gst_rate');
				}else{
					$msg = array('message'=>'Some Error Occurred.', 'class'=>'alert alert-danger' );
					$this->session->set_flashdata('message', $msg);
					redirect('products/editproduct/'.$productId.'?tab=gst_rate');
 			...
 			$this->pktdblib->set_table('product_gst_rates');
 			$productGstRates = $this->pktdblib->get_where_custom('product_id', $productId);
 			$data['product_gst_rates'] = $data['values_posted']['product_gst_rates'] = $productGstRates->result_array();
 		}
 		$data['id'] = $productId;
 		$this->load->view('products/admin_edit_gst_rates', $data);
    }
}
