...
class Order_model extends CI_Model {
    ...
    function invoiceList($postData=null){
        ...
        $searchQuery = "";
        if($searchValue != ''){
           $searchQuery = " AND (t.order_code like '%".$searchValue."%' or t.customer_name like '%".$searchValue."%' or t.invoice_address like'%".$searchValue."%' or t.delivery_address like'%".$searchValue."%' or t.status like '%".$searchValue."%')";
        }
        ## Total number of records without filtering
    ...
    function exportInvoiceList($postData=null){
        ...
        if(NULL!==$postData['fiscal_yr']){
            $this->db->where('oi.fiscal_yr', $postData['fiscal_yr']);
        }
