...
class Orders extends MY_Controller {
    ...
    function export(){
        ...
        if($_SERVER['REQUEST_METHOD']=='POST'){
            ...
            if($dates[0]==$dates[1]){
                ...
                $sql.=' AND o.date BETWEEN "'.date('Y-m-d', strtotime($start->format('Y-m-d'))).'" AND "'.date('Y-m-d', strtotime($end->format('Y-m-d'))).'"';
            }
        }else{
    ...
    function daily_sales_report($data=[]){
        if($this->input->is_ajax_request()){  
            ...
            $zone_no = '';
            if(trim($postData['zone']) != '')
            {
                $zone_no = $postData['zone'];
            }
            
            $sql = 'select l.id, concat(l.first_name, " ",l.surname) as person_name, l.username from login l INNER JOIN user_roles ur on ur.login_id=l.id left join routes r on r.login_id=l.id where ur.login_id=l.id and ur.role_id=7 AND l.is_active=true';
            ...
            foreach($salePersons as $sKey=>$saleBy){
                ...
                if($orders[0]['order_count']){
                    $sql3 = 'select count(o.id) as app_count from orders o where (o.sale_by="'.$saleBy['id'].'" or o.created_by="'.$saleBy['id'].'")';
                    if($dt[0]!=$dt[1]){
                        $sql3.=' AND o.date between "'.$dt[0].' 23:59:59" AND "'.$dt[1].' 23:59:59"';
                    }else{
                    ...
                    if($dt[0]!=$dt[1]){
                        $sql4.=' AND o.date between "'.$dt[0].' 23:59:59" AND "'.$dt[1].' 23:59:59"';
                    }else{
                    ...
                    if($dt[0]!=$dt[1]){
                        $sql5.=' AND o.date between "'.$dt[0].' 23:59:59" AND "'.$dt[1].' 23:59:59"';
                    }else{
