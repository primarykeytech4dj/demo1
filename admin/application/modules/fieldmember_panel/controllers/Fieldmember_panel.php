...
class Fieldmember_Panel extends MY_Controller {
	...
	/*function admin_index($data=[]){
	 ...
	 function admin_index($data=[]){
		...
		if($this->input->is_ajax_request()){  
			...
			//echo "<pre>"; print_r($data);exit;
			foreach($data['aaData'] as $key=>$v){
				$data['aaData'][$key]['customer_name'] = '<span>'.$v['customer_name'].'</span><br><span class="font-italic">Area : '.$v['area_name'].'</span>';
				$action = '';
				if($v['status']=='Delivered'){
					$action = '<i class="fa fa-check alert-success"></i>';
				}elseif($v['status']=='Cancel'){
				...
				/*}elseif($v['status']=='Assigned Delivery Boy'){
					...
					$action = anchor('fieldmember_panel/view_OrderDetail2/'.base64_encode($v['company_id']."-".$v['bill_type']."-".$v['invoice_no']), '<i class="fa fa-truck"></i>', ['class'=>'btn btn-danger']);
				}
				$amount = $this->getInvoiceWiseOrders($v['company_id']."-".$v['bill_type']."-".$v['invoice_no']);
	...
	function getDeliveryBoy($roleId = 6){
	    //echo "reached heer";exit;
	    $deliveryBoys['options'] = [0=>'Select Delivery Boy'];
	    $sql2 = 'Select concat(l.first_name, " ", l.surname," | ",e.emp_code) as employee, l.id, e.is_active from login l inner join user_roles ur on ur.login_id=l.id and ur.account_type="employees" inner join employees e on e.id=ur.user_id where ur.role_id='.$roleId;
        if(!in_array(1, $this->session->userdata('roles')) && !in_array(2, $this->session->userdata('roles'))){
	...
	public function getInvoiceWiseOrders($invno){
		...
		if(count($ex)==3){
	        $sql.= ' AND inv.bill_type='.$ex[1].' and inv.company_id='.$ex[0];
	    }
	    //echo $sql;
		...
		$id = implode(",",$orderId);
		//echo $id;
		$odSql = 'select sum(o.amount_after_tax) as amount_after_tax, sum(o.amount_before_tax) as amount_before_tax, concat(c.first_name," ", c.surname) as customer_name  from orders o  join customers c on c.id=o.customer_id where o.id in ('.$id.') and o.is_active!=0';
		//echo $odSql;
		$order = $this->pktdblib->custom_query($odSql);
		//print_r($order);exit;
		...
		$sql2 = 'Select od.*, p.product, p.base_uom, p.base_price, p.product_type, pi.image_name_1, b.brand from order_details od inner join products p on p.id=od.product_id left join brand_products bp on bp.product_id=p.id left join manufacturing_brands b on b.id=bp.brand_id left join product_images pi on pi.product_id=p.id and pi.featured_image=true where od.is_active=true and od.order_id in ('.$id.')';
		//echo $sql2;
		$ods = $this->pktdblib->custom_query($sql2);
		//echo '<pre>';print_r($ods);
        foreach ($ods as $odkey => $od) {
            ...
            $ods[$odkey]['model'] = isset($productAttribute[0]['model'])?$productAttribute[0]['model']:'';
            $od['base_uom'] = trim($od['base_uom']);
            if($od['base_uom']==0 || empty($productAttribute)){
                $ods[$odkey]['qty_count'] = $ods[$odkey]['qty'] = $od['qty'];
            }else{
	        ...
	        if(!empty($orderDetailArray)){
	            ...
	            if($updDetails){
	            	...
	            	foreach ($orderId as $orderKey => $orderValue) {
		            	...
		            	//$updateDeliveryArray['modified_by'] = $this->session->userdata('user_id');
		            	$updateDeliveryArray['delivery_remark'] = $postData['orders']['delivery_remark'];
		            	
		            	$this->pktdblib->set_table('deliveryboy_order');
		            	$updateDelivery = $this->pktdblib->_update($postData['invoice']['id'],$updateDeliveryArray);
		            	
	            	
	            	if($this->input->post('data[orders][order_status_id]')==5){
		          
			            $postData = $this->input->post();
			            $order_payment=$this->recieve_order_payment($id);
			            if($order_payment){
			                $msg = array('message' => 'Order Updated Successfully','class' => 'alert alert-success');
			                $this->session->set_flashdata('message',$msg);
			            }else{
	...
	function deliveryList($data=[]){
		if($this->input->is_ajax_request()){  
			...
			/*echo '<pre>';
			print_r($postData);*/
			$postData['fiscal_yr'] = $this->pktlib->current_fiscal_year();
			$sql='select min(created) as from_date from deliveryboy_order where 1=1';
			...
			if(in_array(1, $this->session->userdata('roles')) || in_array(2, $this->session->userdata('roles'))){
				...
				$postData['condition']['dbo.employee_id'] = $this->session->userdata('user_id');
				$sql.=' and employee_id='.$this->session->userdata('user_id');
			}
			...
			if(!isset($postData['from_date'])){
			    ...
			    /*echo '<pre>';
			    print_r($query);*/
			    $postData['from_date'] = (NULL!==$query[0]['from_date'])?date('d/m/Y H:i:s', strtotime($query[0]['from_date'])):date('d/m/Y', strtotime('-1 day'));
			    $postData['to_date'] = (NULL!==$query[0]['from_date'])?date('d/m/Y', strtotime('+1 day'. $query[0]['from_date'])):date('d/m/Y');
			}
			//echo "<pre>"; print_r($postData);echo "</pre>";exit;
			$data = $this->fieldmember_panel_model->orderAssignedList2($postData);
			//echo "<pre>"; print_r($data);exit;
			foreach($data['aaData'] as $key=>$v){
				$action = '';
				/*if($v['status']=='Delivered'){
				...
				$amount = $this->getInvoiceWiseOrders($v['company_id']."-".$v['bill_type']."-".$v['invoice_no']);
				$data['aaData'][$key]['amount'] = $amount['amount_after_tax'];
				//$action.=$action;exit;
			}	
			echo json_encode($data);
			exit;
		 ...
		 $data['title'] = "ERP :: Delivery Module";
		 $data['meta_description'] = "Delivery Module";
		 
		 $data['modules'][] = "fieldmember_panel";
		 $data['methods'][] = "deliveryListView";
		 
		 echo Modules::run("templates/fieldmember_template", $data);
	 }
 
	 function deliveryListView($data=[]){
		 $data['formTitle'] = "Delivery Invoice";
		 $data['deliveryBoys'] = Modules::run("Fieldmember_panel/getDeliveryBoy");
		 $data['option']['status'] = [''=>'Select Status', '9'=>'Waiting To Be Delivered', '5'=>'Delivered', 2=>'Cancelled'];
		 $fiscalYr = $this->pktlib->current_fiscal_year();
		 //echo $fiscalYr;exit;
		 $deliveryNos = $this->pktdblib->custom_query('Select distinct delivery_no, created from deliveryboy_order where fiscal_yr like "'.$fiscalYr.'" order by created DESC');
		 $data['option']['deliveryNo'] = [''=>'-Select-'];
		 if(count($deliveryNos)){
		     foreach($deliveryNos as $ky=>$deliveryNo){
		         $data['option']['deliveryNo'][$deliveryNo['delivery_no']] = $deliveryNo['delivery_no'].' ('.date('d/m/Y', strtotime($deliveryNo['created'])).')';
		     }
		 }
		 //echo '<pre>';print_r($data['option']);exit;
		 //print_r($data['option']['delivery_no']);exit;
		 $this->load->view("fieldmember_panel/deliveryListReport", $data);
	}
	
	/*function recieve_order_payment($invoiceId){
        $orderPayments = $this->input->post('data[order_payments]');
        
	    $data = [];
        foreach($orderPayments as $oKey=>$orderPayment):
            //print_r($orderPayment);
            if(!empty(trim($orderPayment['amount'])) && $orderPayment['amount']>0){
        		$data[$oKey]['date'] = $data[$oKey]['created_at']     =  date('Y-m-d H:i:s');
        		$data[$oKey]['created_by']     = $this->session->userdata('user_id');
        		$data[$oKey]['payment_mode']   = $orderPayment['payment_mode'];
        		$data[$oKey]['amount']         = $orderPayment['amount'];
        		$data[$oKey]['invoice_no']         = $invoiceId;
        ...
        if(!empty($data)){
    		$this->pktdblib->set_table("order_payments");
    		$count = $this->pktdblib->_insert_multiple($data);
    		return $count;
        }else{
            return true;
        }
	...
	function view_OrderDetail2($id=NULL){
	    ...
	    $orderIds = $id;
	    $invNo = base64_decode($id);
	    $companyId = 1;
	    $billType = 1;
	    $params = explode("-", $invNo);
	    //print_r($params);exit;
		if(count($params)>1){
		    $id = $params[2];
		    $companyId = $params[0];
	    ...
	    if($_SERVER['REQUEST_METHOD'] == 'POST') {
	        ...
	        if(!empty($orderDetailArray)){
	            ...
	            if($updDetails){
	            	$status = $postData['orders']['order_status_id'];
	            	$orderId = Modules::run('orders/getInvoiceWiseOrderId', $invNo);
	            	//print_r($orderId);exit;
	            	foreach ($orderId as $orderKey => $orderValue) {
	            		//echo $orderValue['id'].'<br>';exit;
	            		$order = Modules::run('orders/updateOrderIdWise', $orderValue['id'], $status);
	            	...
	            	//if($postData['orders']['order_status_id'] != 1){
		            	...
		            	$updateDeliveryArray['modified'] = date('Y-m-d H:i:s');
		            	$this->pktdblib->set_table('deliveryboy_order');
		            	$updateDelivery = $this->pktdblib->custom_query('Update deliveryboy_order set order_status_id='.$postData['orders']['order_status_id'].', modified="'.date('Y-m-d H:i:s').'", delivery_remark="'.$postData['orders']['delivery_remark'].'" where CONCAT(company_id,"-",bill_type,"-", invoice_no)="'.$invNo.'"');
	            	//}
	            	
	            	if($this->input->post('data[orders][order_status_id]')==5){
		                $postData = $this->input->post();
			            $order_payment=$this->recieve_order_payment($invoiceId, $companyId, $billType);
			        
			            if($order_payment){
			                $msg = array('message' => 'Order Updated Successfully. Invoice No: '.$invoiceId.' '.anchor('orders/print_invoice/'.base64_encode($invNo), '<i class="fa fa-print"> </i>', ['target'=>'_new']),'class' => 'alert alert-success');
			                $this->session->set_flashdata('message',$msg);
			            }else{
			                $msg = array('message' => 'Some Error Occurred','class' => 'alert alert-danger');
			                $this->session->set_flashdata('message',$msg);
		                ...
		                $msg = array('message' => 'Order Updated Successfully. Invoice No: '.$invoiceId.' '.anchor('orders/print_invoice/'.base64_encode($invNo), '<i class="fa fa-print"> </i>', ['target'=>'_new']),'class' => 'alert alert-success');
			            $this->session->set_flashdata('message',$msg);
			            redirect('fieldmember_panel/make_delivery2/'.$orderIds);
		            }
	            }
	        }
		...
		$orderStatus = $status->result_array();
		//print_r($orderStatus);exit;
		$data['id'] = $id;//$orderStatus[0]['id'];
		$data['orders']['orders_status_id'] = $orderStatus[0]['order_status_id'];
        //echo $invNo;
		$data['order'] = $this->getInvoiceWiseOrders($invNo);
	...
	function recieve_order_payment($invoiceId, $companyId=1, $billType=1){
        $orderPayments = $this->input->post('data[order_payments]');
	    $data = [];
	    /*echo '<pre>';
        ...
        foreach($orderPayments as $oKey=>$orderPayment):
            ...
            if(!empty(trim($orderPayment['amount'])) && $orderPayment['amount']>0){
        		...
        		$data[$oKey]['amount']         = $orderPayment['amount'];
        		$data[$oKey]['invoice_no']     = $invoiceId;
        		$data[$oKey]['company_id']     = $companyId;
        		$data[$oKey]['bill_type']      = $billType;
            }
