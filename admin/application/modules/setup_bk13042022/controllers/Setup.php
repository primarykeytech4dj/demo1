...
class Setup extends MY_Controller {
	...
	function getBillType(){
	    $billType = 'GST';
	    if((NULL!==$this->input->get('billtype') && $this->input->get('billtype')==101) || (NULL!==$this->input->post('billtype') && $this->input->post('billtype')==101)){
	        $billType = 101;
	    }
	...
	function loadModuleWiseSetup($module=[]){
	    $this->pktdblib->set_table('setup');
	    $query = $this->pktdblib->get_where_custom('module_name', $module['module_name']);
	    $result = $query->result_array();
	    $response = [];
	    foreach($result as $key=>$res){
	        $response[$res['parameter']] = $res['value'];
	    }
	    return $response;
	}
