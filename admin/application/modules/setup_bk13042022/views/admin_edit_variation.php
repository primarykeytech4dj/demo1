	...
	<div class="row">
		<div class="col-md-12">
			...
			<div class="nav-tabs-custom">
				...
				<div class="tab-content">
					<?php echo form_open_multipart('setup/variationsetup', ['class'=>'form-horizontal', 'id'=>'new_product']);
								...
								<div class="box-body" style="overflow-x:scroll">
									<table class="table" id="target">
										...
										<?php if(count($values_posted)>0){
											foreach ($values_posted as $key => $variation) {
		$name  = array(
				'id' =>	'name_'.$key,
				'required'	=>	'required',
				'class'	=>	'form-control select2 viewInput',
		...
		$input['is_active'] = array(
						...
						"value" => true,
													...
													<td><input type="hidden" name="variations[<?=$key?>][id]" value="<?=isset($variation['id'])?$variation['id']:''?>" id="id_<?=$key?>" class="form-control"><?php echo form_dropdown("variations[".$key."][name]", $option['variation'],isset($variation['name'])?$variation['name']:'' ,$name); ?>
														<input type="text" name="variations[<?=$key?>][name_input]" class="form-control input" style="display: none;" id="name_input_<?=$key?>" placeholder='Variation Not found? Enter here' ">
													</td>
													<td><input type="text" name="variations[<?=$key?>][value]" class="form-control" value="<?=$variation['value']?>" required id="value_<?=$key?>" ></td>
													<td><input type="checkbox" name="variations[<?=$key?>][is_active]" class="flat-red" id="is_active_<?=$key?>" 
														<?=($variation['is_active'])?'checked="checked"':''?>></td>
													<td></td>
												</tr>	
													<?php }
												...
												<tr id="0">
													...
													<td><input type="checkbox" name="variations[0][is_active]" class="flat-red" id="is_active_0"></td>
													<td></td>
												</tr>
												<?php } ?>
											</tbody>
