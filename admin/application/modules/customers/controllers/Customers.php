...
class Customers extends MY_Controller {
	...
	function __construct() {
		...
		$this->apiurl = 'http://localhost/restfulapi';
    				...
    				if($this->edit_customer($id, $postData)) {
    				    ...
    				    if(($_SESSION['application']['multiple_company']) && NULL!==($this->input->post("data[companies_customers][company_id]"))){
                            ...
                            if($_SESSION['application']['is_erpnext']){
                               ...
                               $datas['id'] = $id;
                                    $erp = Modules::run('erpnext/update_customers', $datas);
...
<TALLYMESSAGE xmlns:UDF="TallyUDF">
    ...
    function export(){
        // echo '<pre>';
        // $login_id = $this->pktdblib->custom_query('Select login_id from routes where zone_no=1 AND route_no = 3');
        // print_r($login_id[0]['login_id']);echo '/';
        // $salesMan =  $this->pktdblib->custom_query('Select concat(first_name," ",surname) as sales_man from login where id = '.$login_id[0]['login_id']);
        // print_r($salesMan[0]['sales_man']);
        // echo 'hi';exit;
        if($_SERVER['REQUEST_METHOD']=='POST') {
            // echo'<pre>';
            // print_r($this->input->post());exit;
            $zones = $this->input->post('data[zone]');
            $routes = $this->input->post('data[routes]');

            // print_r($filter);exit;
            // $sql1 =  'SELECT customer_id FROM customer_zones WHERE 1=1'.$filter.'';
            $sql = 'Select c.*, concat(c.first_name," ",c.middle_name," ",c.surname) as contact_person, concat(l.first_name," ",l.surname) as created_person, r.route_no, r.zone_no, concat(l2.first_name," ",l2.surname) as sales_man from customers c left join login l on l.id=c.created_by';
            $sql .= ' inner join customer_zones cz on c.id=cz.customer_id';
            $sql .= ' left join routes r on r.zone_no=cz.zone_no and r.route_no=cz.route_no';
            $sql .= ' left join login l2 on l2.id=r.login_id';

            $sql.=' where 1=1';
            if(NULL !== $zones && !empty($zones))
            {
               $zone = implode(', ', $zones);
               $sql.= ' AND cz.zone_no IN ('.$zone.')';
            }
            if(NULL !== $routes && !empty($routes))
            {
               $route = implode(', ', $routes);
               $sql.= ' AND cz.route_no IN ('.$route.')';
            }
            $sql.=' order by c.created DESC, c.first_name ASC';
            //print_r($sql);exit;
            $query = $this->pktdblib->custom_query($sql);
            // print_r($query);exit;
            // print_r($filter);exit;
            $chkzone = $this->pktdblib->custom_query('SHOW TABLES LIKE "customer_zones"');
        $chkroute = $this->pktdblib->custom_query('SHOW TABLES LIKE "routes"');
        ...
        $tableColumns = ['Zone No', 'Route No', 'Sales Man','Customer Id', 'App Order Count' ,'Customer Code', 'Shop Name/ Tally Name', 'Contact Person', 'Billing Name', 'Email Id', 'Contact 1', 'Referral Code', 'Entered By', 'Registered On', 'Status', 'Address', 'Area', 'Pincode'];
        ...
        $excel->getActiveSheet()->getColumnDimension('I')->setWidth(38);
        $excelRow = 2;
        //echo '<pre>';
        foreach($query as $customer){

            $orderSql =  'select count(id)  as app_order_count FROM orders where order_status_id NOT IN (1,2,8) AND customer_id = '.$customer['id'].'';
			$countResult = $this->pktdblib->custom_query($orderSql);
            ...
            $col = 0;
            // if(!empty($chkzone)){
            //     // $czone = $this->pktdblib->custom_query('Select customer_zones.route_no, r.route_name, customer_zones.zone_no,concat(first_name," ",surname) as sales_man from customer_zones left join routes r on r.zone_no=customer_zones.zone_no and r.route_no=customer_zones.route_no left join login l on l.id=r.login_id where customer_zones.customer_id='.$customer['id']);
            ...
            //         $excel->getActiveSheet()->setCellValueByColumnAndRow(++$col, $excelRow, $czone[0]['sales_man']);
            ...
            $excel->getActiveSheet()->setCellValueByColumnAndRow(++$col, $excelRow, $customer['sales_man']);
            
            $excel->getActiveSheet()->setCellValueByColumnAndRow(++$col, $excelRow, $customer['id']);

            /*if(!empty($czone[0]['zone_no']) && !empty($czone[0]['route_no'])){
                $login_id = $this->pktdblib->custom_query('Select login_id from routes where zone_no='.$czone[0]['zone_no'].' AND route_no = '.$czone[0]['route_no']);
                if(count($login_id)>0)
                {
                    ...
                    if(!empty($salesMan)){
                        ...
                        $excel->getActiveSheet()->setCellValueByColumnAndRow($col, $excelRow, $salesMan[0]['sales_man']);
                    }
                    else{
                        $excel->getActiveSheet()->setCellValueByColumnAndRow($col, $excelRow, '');
                    }
            ...
            }else if(!empty($customer['zone_no']) && !empty($customer['route_no'])){
                $login_id = $this->pktdblib->custom_query('Select login_id from routes where zone_no='.$customer['zone_no'].' AND route_no = '.$customer['route_no']);
                if(count($login_id)>0)
                {
            ...
            if(count($countResult)>0)
            {
                ...
                $excel->getActiveSheet()->setCellValueByColumnAndRow(++$col, $excelRow, $countResult[0]['app_order_count']);
            }
            else{
                $excel->getActiveSheet()->setCellValueByColumnAndRow($col, $excelRow, '');
            }
    ...
    $data['meta_title'] = "ERP";
    $data['title'] = "ERP : Customer Module";
    $data['meta_description'] = "Export Customer XML";
    ...
    $(document).on("change", ".zone",function(){
        ...
        $.ajax({
            ...
            url :  base_url+"customers/get_routes/"+zone,
            success: function(response) {
                console.log(response);
    ...
    function export_form()
    {
        ...
        $this->load->view('customers/export_form', $data);
    }
    ...
    function get_routes($data="")
    {
        ...
        // $response = $data['option']['routes'][$route['route_no']];
        // print_r($response);
        if($this->input->is_ajax_request()){
            echo json_encode($response);
            exit;
            ...
            if(!empty($_FILES)) {
                ...
                if(end($chk_ext)=='xlsx' || end($chk_ext) == 'xls' || end($chk_ext) == 'csv') {
                    ...
                    foreach ($worksheet->getRowIterator() as $wkey=>$row) {
                        ...
                        if($wkey == 1)
                        {
                            $header[$wkey]['product'] = $product;
                            $header[$wkey]['quantity'] = $quantity;
                        }
                        else if ($product == '') {
                    ...
                    $msg = ['status'=>'error', 'message'=>'Invalid file type'];
                    echo json_encode($msg);
                    exit;
                }
            }
    ...
    public function visit_customer_list(){
        // admin_visit_customer_listing
        ...
        if($_SERVER['REQUEST_METHOD']=='POST'){  
			...
			$response = json_decode($this->ajaxListApi($url, 'POST', $postData),true);
            // print_r($response);exit;
            foreach($response['aaData'] as $rkey => $value)
			{
				// $active = ($value['is_active']==true)?'alert-success fa fa-check-circle':'alert-danger fa fa-remove';
				// $response['aaData'][$rkey]['is_active'] = "<i class='".$active."'></i>";
                $response['aaData'][$rkey]['first_name'] = '<a href="#" style="color:black" class="customer_name"><u>'.$value['first_name'].'</u></a><br><span class="italic-font">Area : '.$value['area_name'].'</span>';
                
				$action =  '<div class="input-group">
								<button type="button" class="btn btn-default" style="background-color:#F4813E;color:white;font-weight:bold">
									<span class="fa fa-map-marker " ></span>
								</button>
                            </div>';
				$response['aaData'][$rkey]['action'] = $action;
			}
            echo json_encode($response);
			exit;
		}
        ...
        $data['meta_description'] = "Upload Customers";
        $data['meta_keyword'] = "Upload Customers";
        $data['content'] = 'customers/admin_visit_customer_listing';
        echo Modules::run('templates/admin_template', $data);
    }

    public function get_salesman_route()
    {
        if($_SERVER['REQUEST_METHOD']=='POST'){  
            $routes = [];
            $url1 = $this->apiurl.'/customer/getRoute';
            $postData_['data']['login_id'] = $this->session->userdata('user_id');
            $response = json_decode($this->postApi($url1, 'POST', $postData_),true);
            foreach($response as $eKey=>$route){
                $routes[] = $route['route_no'];
             }
    ...
    public function get_customers_order($id = null)
    {
        if($_SERVER['REQUEST_METHOD']=='POST'){  
			// $postData = [];
			...
			$url = $this->apiurl.'/customer/getOrderList';
			// $postData['customer_id'] = $id;
			$response = json_decode($this->ajaxListApi($url, 'POST', $postData),true);
            echo json_encode($response);
			exit;
    ...
    {
        if($_SERVER['REQUEST_METHOD']=='POST'){  
            $postData['data'] = $this->input->post();
            // print_r($postData['data']);exit;
            $postData['data']['user_id'] = $this->session->userdata('user_id');
            $url = $this->apiurl.'/customer/createCustomerVisitList';
			// $postData['customer_id'] = $id;
			$response = json_decode($this->postApi($url, 'POST', $postData),true);
            echo json_encode($response);
			exit;
        }
    }
    ...
    // ================================================= API Requests ============================================\\
    public function ajaxListApi($url = '', $method='POST', $params = []){
		// echo $this->token;
		// print_r($url. ' '.$method);
		// print_r($params);exit;
		$curl = curl_init();
		curl_setopt_array($curl, array(
		  ...
		  CURLOPT_POST => 1,
		  CURLOPT_HTTPHEADER => array('Authorization: Bearer '.$this->token, 'Content-Type: application/json', 'Accept: application/json'),
		));
      ...
      public function getApi($url = '', $method='POST', $params = [])
	{
		$curl = curl_init();
