...
class Address extends MY_Controller {
		...
		if($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			if($this->form_validation->run()!==FALSE)
			{
				...
				if(count($uid)>1){
					...
					// exit;
					$post_data['data']['address']['created'] = date('Y-m-d H:i:s');
					$post_data['data']['address']['modified'] = date('Y-m-d H:i:s');
				...
				{ 
					/*print_r($user_address);
					exit;*/
					// $2y$10$hXrM3RbnvoiBrlawF70T4eLP02M19W6cdJpHvhITjXoI8W27jbSxa
    ...
    function type_wise_usertest($data = []){
		$userType = '';
		$userId = '';
