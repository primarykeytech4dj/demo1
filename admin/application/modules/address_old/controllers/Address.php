	...
	function admin_edit($id = null){
		...
		if($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			if(empty($error)) { //echo "hii"; print_r($post_data); exit;
				$this->pktdblib->set_table('address');
				$post_data['data']['address']['is_default'] = (isset($post_data['data']['address']['is_default'])?TRUE:FALSE);
				$post_data['data']['address']['is_active'] = (isset($post_data['data']['address']['is_active'])?TRUE:FALSE);
...
function type_wise_user2($data = []){
		...
		/*echo $userType;
		echo $userId;exit;*/
		$sql = 'Select a.id,a.site_name, a.pincode from customers c inner join address a on a.user_id=c.id and a.type="'.$userType.'" where a.user_id='.$userId.' and a.is_active=true';
        $sql2 = $this->pktdblib->custom_query('select * from user_roles where user_id='.$userId.' and account_type="'.$userType.'"');
        if(count($sql2)>0){
            $sql2 = $sql2[0];
            $sql.= ' UNION Select a.id,a.site_name, a.pincode from address a where a.is_active=true and a.user_id='.$sql2['login_id'].' and a.type="login"';
        }
        //echo $sql;
		$typeWiseUsers = $this->pktdblib->custom_query($sql);//exit;
		$userList = !isset($data['is_ajax'])?((count($typeWiseUsers)>0)?[0=>['id'=>0, 'text'=>'Select Address']]:[]):[''=>'Select Address'];
		foreach ($typeWiseUsers as $key => $typeWiseUser) {
			if(isset($data['is_ajax']) && $data['is_ajax']==FALSE){
				$userList[$typeWiseUser['id']] = $typeWiseUser['site_name']." ".$typeWiseUser['pincode'];
			}else{
				$userList[$key+1]['id'] = $typeWiseUser['id'];
