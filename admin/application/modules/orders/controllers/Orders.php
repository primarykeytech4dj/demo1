...
class Orders extends MY_Controller {
	...
	function admin_add_2() {
        ...
        // print_r($id);exit;
            ...
            if(!empty($getData))
            {
                ...
                $data['customer_id'] =  $getData['customer_id'];
            }
        ...
        $data['js'][] = '<script type="text/javascript">
            $(document).on("change", ".product_id",function(){
                ...
                $.ajax({
                    ...
                    dataType: "json",
                    data: "product_id="+productId,
                    url :  base_url+"products/get_product_details_2/"+productId,
                    success: function(response) {
                        console.log(response);

                
                        $("#productattributeid_"+trId).select2("destroy").empty().select2({data : response}).trigger("change");
                  }
            ...
            $("form").on("submit", function(e){
                ...
                var amount_after_tax = parseFloat($("#amount_before_tax").val());
        ...
        //  alert(amount_after_tax < min_order);
                if(amount_after_tax < min_order){
                    alert("Minimun Order Value is "+min_order+"");
    ...
    function admin_order_form2($data = []) {
        ...
        $sql = 'SELECT value FROM setup WHERE module_name = "sales_order" AND parameter = "min_order"';
        $min_order = $this->pktdblib->custom_query($sql);

        $data['min_order'] = $min_order[0]['value'];
    ...
    public function admin_invoice_order_form($data = []){
        ...
        $postData = $this->input->post('filters');
        // $data['zones'] = [];
       
        // print_r($postData);exit;
        $date = '2021-12-06';//date('Y-m-d', strtotime('-50 day'));
        $sql1 = 'Select distinct o.customer_id from orders o';
        $sql1 .= ' LEFT JOIN customer_zones cz ON cz.customer_id = o.customer_id';
        $sql1 .= ' WHERE 1=1';
        if(isset($postData['zone']))
        {
            $zone = implode(', ', $postData['zone']);
            $sql1.= ' AND cz.zone_no IN ('.$zone.')';
            $data['zones'] = $postData['zone'];
        }
        if(isset($postData['routes']))
        {
            $route_no = implode(', ', $postData['routes']);
            $sql1.= ' AND cz.route_no IN ('.$route_no.')';
           
        }
        if(isset($postData['sale_by']))
        {
            if(NULL !== $postData['sale_by'] && !empty($postData['sale_by']))
            {
                $sql1.= ' AND o.sale_by = '.$postData['sale_by'].'';
            }
         
        }
        $sql1 .= ' AND o.order_status_id = 3';
        $sql1 .= ' AND o.order_code NOT IN (Select io.order_code from invoice_orders io)';
        $sql1 .= ' AND o.date > "'.$date.'"';

        $custQuery = $this->pktdblib->custom_query($sql1);
        $customers = [];
                        ...
                        foreach($resultArray as $invKey=>$invoice){
                          ...
                          $salesXml.='<VOUCHER REMOTEID="" VCHKEY="" VCHTYPE="Sales Dry Fruit" ACTION="Create" OBJVIEW="Invoice Voucher View">
                            <ADDRESS.LIST TYPE="String"><€>
                              '.str_replace(', ', '</€><ADDRESS>', htmlspecialchars($invoice['delivery_address'], ENT_QUOTES)).'</ADDRESS>
                            </ADDRESS.LIST>
    ...
    function daily_sales_report($data=[]){
        if($this->input->is_ajax_request()){  
            ...
            foreach($salePersons as $sKey=>$saleBy){
                ...
                if($dt[0]!=$dt[1]){
                    $sql2.=' AND date between "'.$dt[0].' 06:00:00" AND "'.$dt[1].' 06:00:00"';
                }else{
                ...
                echo $sql2.'<br>';*/
                /*$sql2 = 'SELECT sum(amount_after_tax) as sale_amt, count(id) as order_count, concat(l.first_name," ",l.surname) as sales_person FROM `orders` INNER JOIN user_roles ur on ur.login_id=orders.sale_by and role_id=7 INNER JOIN login l on l.id=ur.login_id';
                $sql2.=' INNER JOIN `customer_zones` as `cz` ON orders.customer_id=cz.customer_id';
                $sql2.='  WHERE date BETWEEN "'.$dt[0].' 06:00:00" AND "'.$dt[1].' 06:00:00" AND order_status_id NOT in  (1,2,8)';
                $sql2.=" AND (orders.sale_by=".$saleBy['id'].' OR orders.created_by='.$saleBy['id'].')';*/
                ...
                if($orders[0]['order_count']){
                    ...
                    if($dt[0]!=$dt[1]){
                        $sql3.=' AND o.date between "'.$dt[0].' 06:00:00" AND "'.$dt[1].' 06:00:00"';
                    }else{
                    ...
                    if($dt[0]!=$dt[1]){
                        $sql4.=' AND o.date between "'.$dt[0].' 06:00:00" AND "'.$dt[1].' 06:00:00"';
                    }else{
                    ...
                    if($dt[0]!=$dt[1]){
                        $sql5.=' AND o.date between "'.$dt[0].' 06:00:00" AND "'.$dt[1].' 06:00:00"';
                    }else{
                    ...
                    // print_r($credit_note_amt);exit;
                    /*'total_amt'=>0.00,
                    'average'=>'0.00',
                    'credit_note_count'=>0,
                    'credit_note_amt'=>0.00*/
                }
                $order_count += $data['aaData'][$sKey]['order_count'];
                $customer_count += $data['aaData'][$sKey]['customer_count'];
                $item_count += $data['aaData'][$sKey]['item_count'];
                $total_amt  += $orders[0]['sale_amt'];
                $average += ($orders[0]['sale_amt'] > 0) ? $orders[0]['sale_amt']/$orders[0]['order_count'] : 0.00 ;
                $credit_note_count += $data['aaData'][$sKey]['credit_note_count'];
                $credit_note_amt += $data['aaData'][$sKey]['credit_note_amt'];
            ...
            $data['aaData'][] = [
                ...
                'credit_note_amt'=>number_format($credit_note_amt,2),
    ...
    function sales_report($data=[]){
        ...
        // print_r($postData['zone']);exit;
            $zone_no = "";
            if($postData['zone'] != '')
            {
                $zone_no = json_decode($postData['zone']);
            }
            $routeNo= "";
            if($postData['routes'] != '')
            {
                $routeNo = json_decode($postData['routes']);
            }
            $products = "";
            if($postData['product'] != '')
            {
                $products = json_decode($postData['product']);
            }
            
        


            $sql = "SELECT zone_no,route_name,route_no,login_id FROM routes where 1=1 ";
            if(NULL !== $zone_no && !empty($zone_no))
            {
                $zone = implode(', ', $zone_no);
                $sql.= ' AND zone_no IN ('.$zone.')';
            }
            if(NULL !== $routeNo && !empty($routeNo))
            {
                $route_no = implode(', ', $routeNo);
                $sql.= ' AND route_no IN ('.$route_no.')';
            }
            if(NULL !== $postData['sale_by'] && !empty($postData['sale_by']))
            {
                $sql.= ' AND login_id = '.$postData['sale_by'].'';
            }
            
            // echo '<pre>';
            // print_r($sql);
            $routes = $this->pktdblib->custom_query($sql);
            $order_count = 0;
            $customer_count = 0;
            $item_count = 0;
            $active_customers = 0;
            ...
            $app_count = 0;
            // print_r($routes);exit;
            foreach($routes as $sKey=>$route){
                // print_r($route);exit;
                $sql21 = 'select l.id, concat(l.first_name, " ",l.surname) as person_name, l.username from login l INNER JOIN user_roles ur on ur.login_id=l.id where ur.login_id=l.id and ur.role_id=7 and l.id = '.$route['login_id'].'';
                // print_r($sql3);
                $salePersons = $this->pktdblib->custom_query($sql21);
                // print_r($sql21);exit;
                $data['aaData'][$sKey] = [
                    'sr_no'=>$sKey+1,
                    ...
                    'zone' => $route['zone_no'],
                    'app_count'=>0,
                    'salesman' => $salePersons[0]['person_name'],
                    'customer_count'=>0,
                    'emarkit'=>0,
                    'active_customers'=>0,
                    'item_count'=>0,
                ...
                $sql2 = 'SELECT count(o.id) as order_count, sum(o.amount_after_tax) as sale_amt, o.sale_by';
                // $sql2.=' FROM `orders` WHERE 1=1';
                $sql2.=' FROM `orders` as `o`';
                $sql2.=' LEFT JOIN `customer_zones` as `cz` ON o.customer_id=cz.customer_id';
                $sql2.=' WHERE 1=1';
                $sql2.= ' AND cz.route_no = '.$route['route_no'].'';

                if($dt[0]!=$dt[1]){
                    $sql2.=' AND o.date between "'.$dt[0].' 06:00:00" AND "'.$dt[1].' 06:00:00"';
                }else{
                    $sql2.=' AND o.date LIKE "'.$dt[0].'%"';
                }
                // $sql2.=' AND o.project_name NOT IN ("POS", "Salesman POS")';
                $sql2.=' AND o.order_status_id NOT IN (1,2,8)';
                if(NULL !== $products && !empty($products))
                {
                    $product = implode(', ', $products);
                $sql2.= ' AND o.id IN (SELECT DISTINCT order_id FROM order_details WHERE product_id IN ('.$product.') )';
                }
                $orders = $this->pktdblib->custom_query($sql2);
                // print_r($sql2);exit;
                ...
                $sql3 = 'select count(o.id) as app_count from orders o';
                $sql3.=' INNER JOIN `customer_zones` as `cz` ON o.customer_id=cz.customer_id';
                $sql3.=' WHERE 1=1';
                $sql3.= ' AND cz.route_no = '.$route['route_no'].'';
                //(o.sale_by="'.$saleBy['id'].'" or o.created_by="'.$saleBy['id'].'")
                if($dt[0]!=$dt[1]){
                    $sql3.=' AND o.date between "'.$dt[0].' 06:00:00" AND "'.$dt[1].' 06:00:00"';
                }else{
                ...
                {
                    $product = implode(', ', $products);
                    $sql3.= ' AND o.id IN (SELECT DISTINCT order_id FROM order_details WHERE product_id IN ('.$product.') )';
                }
                //echo $sql3.'<br>';
                $appCount=$this->pktdblib->custom_query($sql3);
                // print_r($appCount);exit;
                //     /*customer count*/
                $sql4 = 'select count(distinct o.customer_id) as customer_count from orders o';
                $sql4.=' INNER JOIN `customer_zones` as `cz` ON o.customer_id=cz.customer_id';
                $sql4.=' WHERE 1=1';
                $sql4.= ' AND cz.route_no = '.$route['route_no'].'';
                if($dt[0]!=$dt[1]){
                    $sql4.=' AND o.date between "'.$dt[0].' 06:00:00" AND "'.$dt[1].' 06:00:00"';
                }else{
                    $sql4.=' AND o.date LIKE "'.$dt[0].'%"';
                ...
                echo $sql4;*/
                $custCount=$this->pktdblib->custom_query($sql4);
                // print_r($custCount);exit;
                $sql5 = 'select count(distinct od.product_id) as item_count, sum(od.unit_price*od.return_quantity) as credit_note_amt, sum(return_quantity) as return_qty from orders o inner join order_details od on od.order_id=o.id AND od.is_active=true';
                $sql5.=' INNER JOIN `customer_zones` as `cz` ON o.customer_id=cz.customer_id';
                
                $sql5.=' WHERE 1=1';
                $sql5.= ' AND cz.route_no = '.$route['route_no'].'';
                if($dt[0]!=$dt[1]){
                    $sql5.=' AND o.date between "'.$dt[0].' 06:00:00" AND "'.$dt[1].' 06:00:00"';
                }else{
                    $sql5.=' AND o.date LIKE "'.$dt[0].'%"';
                ...
                {
                    $product = implode(', ', $products);
                    $sql5.= ' AND o.id IN (SELECT DISTINCT order_id FROM order_details WHERE product_id IN ('.$product.') )';
                }
                ...
                $sql6 = ' Select count(c.id) as active_customers FROM customers c';
                $sql6.=' INNER JOIN `customer_zones` as `cz` ON c.id=cz.customer_id';
                $sql6.=' WHERE 1=1';
                $sql6.= ' AND cz.route_no = '.$route['route_no'].'';
                if($dt[0]!=$dt[1]){
                    $sql6.=' AND c.created between "'.$dt[0].' 06:00:00" AND "'.$dt[1].' 06:00:00"';
                }else{
                    $sql6.=' AND c.created LIKE "'.$dt[0].'%"';
                }
                $sql6.=' AND c.is_active = 1';
                
                $activeCustomers=$this->pktdblib->custom_query($sql6);
                // print_r($sql6);
                // print_r($activeCustomers);exit;
                $data['aaData'][$sKey]['order_count'] = $orders[0]['order_count'];
                // $data['aaData'][$sKey]['salesman'] = (!empty($salePersons))?$salePersons[0]['person_name']:'Not defined';
                $data['aaData'][$sKey]['app_percent'] = ($orders[0]['order_count']>0)?number_format(($appCount[0]['app_count']/$orders[0]['order_count'])*100.00, 2, '.', ''):'0';
                $data['aaData'][$sKey]['emarkit'] = $appCount[0]['app_count'];
                $data['aaData'][$sKey]['customer_count'] = $custCount[0]['customer_count'];
                $data['aaData'][$sKey]['item_count'] = $itemCount[0]['item_count'];
                $data['aaData'][$sKey]['active_customers'] = $activeCustomers[0]['active_customers'];
                //     $data['aaData'][$sKey]['credit_note_amt'] = $itemCount[0]['credit_note_amt'];
                $data['aaData'][$sKey]['average'] = ($orders[0]['order_count']>0)?number_format($orders[0]['sale_amt']/$orders[0]['order_count'],2):0;
                $data['aaData'][$sKey]['total_amt'] = number_format($orders[0]['sale_amt'],2);
            ...
            // }
            $order_count +=  $data['aaData'][$sKey]['order_count'];
            $app_count += $data['aaData'][$sKey]['emarkit'];    
            $customer_count += $data['aaData'][$sKey]['customer_count'];
            $item_count += $data['aaData'][$sKey]['item_count'];
            $active_customers += $data['aaData'][$sKey]['active_customers'];
            $average += ($orders[0]['sale_amt'] > 0) ? $orders[0]['sale_amt']/$orders[0]['order_count'] : 0.00 ;
            $total_amt  += $orders[0]['sale_amt'];
            ...
            // print_r($appCount[0]['app_count']);exit;
            /*$data["iTotalRecords"]=0;
            $data["iTotalDisplayRecords"]="0";*/
            // print_r($data);exit;
            echo json_encode($data);
        ...
        $data['title'] = "ERP : Orders";
        $data['meta_description'] = "Sales Report";
        
        $data['modules'][] = "orders";
        $data['methods'][] = "report_view";
        $data['js'][] = '<script type="text/javascript">
        $(document).on("change", ".zone",function(){
    ...
    function report_view($data=[]){
        $zones =$this->pktdblib->custom_query('SELECT DISTINCT zone_no FROM routes');
        foreach($zones as $eKey=>$zone){
           $data['option']['zone'][$zone['zone_no']] = $zone['zone_no'];
        }

        $products = $this->pktdblib->custom_query('SELECT id, product FROM products');
        foreach($products as $pKey=>$product){
            $data['option']['product'][$product['id']] = $product['product'];
         }
        $data['salePersons'] = $this->pktdblib->custom_query('select l.id, concat(l.first_name, " ",l.surname) as person_name, l.username from login l INNER JOIN user_roles ur on ur.login_id=l.id where ur.login_id=l.id and ur.role_id=7 AND l.is_active=true');
        $data['formTitle'] = "Sales Report";
        $this->load->view("orders/sales_report", $data);
    }

    function get_routes($data="")
    {
        // print_r($data);
        $routes =$this->pktdblib->custom_query('SELECT route_no FROM routes WHERE zone_no IN ('.$data.')');
        // print_r($routes);
        foreach($routes as $eKey=>$route){
           $response[] = $route['route_no'];
        }
        if($this->input->is_ajax_request()){
    ...
    public function export_weekly_report(){
        ...
        if($_SERVER['REQUEST_METHOD']=='POST'){  
            ...
            header('Content-Disposition: attachment;filename="12_WEEKS_Report.xlsx"');
            header('Cache-Control: max-age=0');
            $spreadsheet = new \PhpOffice\PhpSpreadsheet\Spreadsheet;
            
            $pw = strtotime("-11 week +1 day");
            $sw = strtotime("last monday midnight",$pw);
            $sw = date("Y-m-d",$sw);


            //echo $sw.' 00:00:00 '.date('Y-m-d').' 23:59:59<br>' ;
            $customerWiseData = [];
            $dateArray = [];
            // echo '<pre>';
            $customer_id = [];
            for($i=0;$i<12;$i++){
                ...
                $dateArray[] = $end_week;
                $sql =  'select COUNT(o.id) as count, o.project_name, o.customer_id, SUM(amount_after_tax) as amount, concat(c.first_name," ",c.middle_name, c.surname) as contact_person, c.company_name from orders o ';
                $sql .= ' INNER JOIN customers c on c.id=o.customer_id';
                $sql .= ' LEFT JOIN customer_zones as cz ON cz.customer_id=c.id';
                $sql .= ' WHERE o.date BETWEEN "'.$start_week.' 00:00:00" AND "'.$end_week.' 23:59:59"';
                if(isset($postData['zone']))
                {
                    $zone = implode(', ', $postData['zone']);
                    $sql.= ' AND cz.zone_no IN ('.$zone.')';
                }
                if(isset($postData['routes']))
                {
                    $route_no = implode(', ', $postData['routes']);
                    $sql.= ' AND cz.route_no IN ('.$route_no.')';
                }
                if(isset($postData['sale_by']))
                {
                    if(NULL !== $postData['sale_by'] && !empty($postData['sale_by']))
                    {
                        $sql.= ' AND o.sale_by = '.$postData['sale_by'].'';
                    }
                }
                $sql .= ' AND o.order_status_id NOT IN (1,2,8)';
                $sql .= ' GROUP BY o.customer_id, o.project_name, o.sale_by';
                $sql .= ' ORDER by trim(c.company_name) ASC';

                $orderData = $this->pktdblib->custom_query($sql);
                foreach($orderData as $okey=>$order){

                   
                    
                    
                    if(!array_key_exists($order['customer_id'], $customerWiseData)){
                        $customerWiseData[$order['customer_id']]=[
                            'contact_name'=>trim($order['contact_person']),
                            'company_name'=>trim($order['company_name']),
                            'app'=>0,
                            'pos'=>0,
                            'return_amt'=>0,
                            'return_nos'=>0,
                        ...
                        $customerWiseData[$order['customer_id']]['week_count'][$end_week] = 0;

                        
                    }else{
                        if(!array_key_exists($end_week, $customerWiseData[$order['customer_id']]['week_count'])){
                            $customerWiseData[$order['customer_id']]['week_count'][$end_week] = 0;
                        }
                        
                       
                    }
                    $appName = $this->clubSaleFrom($order['project_name']);
                    
                    $customerWiseData[$order['customer_id']][$appName] = $customerWiseData[$order['customer_id']][$appName]+$order['count'];
                    $customerWiseData[$order['customer_id']]['week_count'][$end_week]+= $order['amount'];
                    // if($order['customer_id']==783){
                    //     print_r($order);
                    //     echo $appName." ".$order['project_name']." ".$end_week." ". $customerWiseData[$order['customer_id']]['week_count'][$end_week]."<br>";
                    // }

                    // print_r($customerWiseData);
            ...
            $sql2 = 'SELECT o.customer_id, sum(od.unit_price*od.return_quantity) as sum_return_amt, sum(od.return_quantity) as return_nos FROM orders as o INNER JOIN order_details as od ON od.order_id=o.id WHERE od.return_quantity > 0 AND o.customer_id IN ('.implode(', ',$customer_id).')  AND o.date BETWEEN "'.$sw.' 00:00:00" and "'.date('Y-m-d').' 23:59:59" and o.order_status_id NOT IN (1,2,8) GROUP BY o.customer_id';
            $returnData = $this->pktdblib->custom_query($sql2);
            
            $return_array = [];
            foreach($returnData as $rkey => $return)
            {
                $return_array[$return['customer_id']] = [
                    'sum_return_amt' =>  $return['sum_return_amt'],
                    'return_nos' => $return['return_nos']
            ...
            // echo '<pre>';
            // print_r($customerWiseData[783]);exit;
            ...
            $col++;
            $sheet->setCellValueByColumnAndRow($col, $row, 'Customer Name');
            $col++;
            foreach($dateArray as $dKey => $date)
            {
                
                $sheet->setCellValueByColumnAndRow($col, $row, date('Y-M-d', strtotime($date)));
                $col++;
                
            }
            $sheet->setCellValueByColumnAndRow($col, $row, 'Total Amt');
            $col++;
            $sheet->setCellValueByColumnAndRow($col, $row, 'App Orders');
            $col++;
            $sheet->setCellValueByColumnAndRow($col, $row, 'POS Orders');
            $col++;
            $sheet->setCellValueByColumnAndRow($col, $row, 'Return Quantity');
            $col++;
            $sheet->setCellValueByColumnAndRow($col, $row, 'Return Amt');
            $row++;
            
            // $row = 2;
            foreach($customerWiseData as $cKey => $cutomer)
            {
                $col = 0;
                $col++;
                $sheet->setCellValueByColumnAndRow($col, $row, $cutomer['contact_name'].'('.$cKey.')');
                $col++;
                $total_amt = 0;
                foreach($dateArray as $dKey => $date)
                {
                    $amt = 0;
                    if(isset($cutomer['week_count'][$date])){
                        $sheet->setCellValueByColumnAndRow($col, $row, $cutomer['week_count'][$date]);
                        $amt = $cutomer['week_count'][$date];
                    }
                    else{
                ...
                $sheet->setCellValueByColumnAndRow($col, $row, $total_amt);
                $col++;
                $sheet->setCellValueByColumnAndRow($col, $row, $cutomer['app']);
                $col++;
                $sheet->setCellValueByColumnAndRow($col, $row, $cutomer['pos']);
                $col++;
                if(isset($return_array[$cKey]))
                {
                    $return_nos = $return_array[$cKey]['return_nos'];
                    $sum_return_amt = $return_array[$cKey]['sum_return_amt'];
                }
                else{
                    $return_nos = 0;
                    $sum_return_amt = 0;
                }
                $sheet->setCellValueByColumnAndRow($col, $row, $return_nos);
                $col++;
                $sheet->setCellValueByColumnAndRow($col, $row, $sum_return_amt);
                $row++;
            }
        ...
        $data['title'] = "ERP : Orders";
        $data['meta_description'] = "Weekly Sales Report";
        
        $data['modules'][] = "orders";
        $data['methods'][] = "weekly_report_view";
        $data['js'][] = '<script type="text/javascript">
        $(document).on("change", ".zone",function(){
    ...
    public function weekly_report_view($data=[])
    {
        $zones =$this->pktdblib->custom_query('SELECT DISTINCT zone_no FROM routes');
        foreach($zones as $eKey=>$zone){
           $data['option']['zone'][$zone['zone_no']] = $zone['zone_no'];
        ...
        $data['salePersons'] = $this->pktdblib->custom_query('select l.id, concat(l.first_name, " ",l.surname) as person_name, l.username from login l INNER JOIN user_roles ur on ur.login_id=l.id where ur.login_id=l.id and ur.role_id=7 AND l.is_active=true');
        $data['formTitle'] = "Weekly Sales Report";
        $this->load->view("orders/weekly_report", $data);

    }



    function clubSaleFrom($param){
        $app = [
            'app'=>[
                'Order Through App', 'Order Through App Version 3', 'Order Through App Version 2', 'App'
            ],
            'pos'=>[
                'POS', 'Salesman POS'
            ]
        ...
        $appkey = array_search($param, $app['app']);
        if($appkey !== FALSE)
        {
            $var='app';
        }
        $poskey = array_search($param, $app['pos']);
        if($poskey !== FALSE)
        {
            $var='pos';
        }
    ...
    public function export_weekly_report2(){
        ...
        if($_SERVER['REQUEST_METHOD']=='POST'){  
            ...
            $custo = [];
            $salePerson = [];
            $query = 'SELECT c.id as id, cz.zone_no as zone, cz.route_no as route_no, r.route_name as route, concat(c.first_name ," ",c.surname) as name , concat(a.address_1," ", a.address_2) as address FROM customers as c';
            $query .= ' INNER JOIN login as l ON l.employee_id=c.id';
            $query .= ' INNER JOIN address as a ON a.user_id = c.id AND is_default = 1';
            $query .= ' LEFT JOIN customer_zones as cz ON cz.customer_id=c.id';
            $query .= ' LEFT JOIN routes as r ON r.route_no = cz.route_no';
            $query .= ' WHERE l.is_active != 0';
            
     
            if(isset($postData['zone']))
            {
                $zone = implode(', ', $postData['zone']);
                $query.= ' AND cz.zone_no IN ('.$zone.')';
            }
            if(isset($postData['routes']))
            {
                $route_no = implode(', ', $postData['routes']);
                $query.= ' AND cz.route_no IN ('.$route_no.')';
            }
            
            $customers = $this->pktdblib->custom_query($query);

            $salesPerson = $this->pktdblib->custom_query('select l.id, concat(l.first_name, " ",l.surname) as person_name, l.username from login l INNER JOIN user_roles ur on ur.login_id=l.id where ur.login_id=l.id and ur.role_id=7 AND l.is_active=true');
            foreach($salesPerson as $spKey => $sales)
            {
                $salePerson[$sales['id']]['person_name'] = $sales['person_name'];
                $salePerson[$sales['id']]['username'] = $sales['username'];
            }
            ...
            for($i=0;$i<12;$i++){
                ...
                $end_week = date("Y-m-d",$end_week);
                $dateArray[] = $end_week;
                $sql =  'select o.sale_by as salesman, COUNT(o.id) as count, o.project_name, o.customer_id, SUM(amount_after_tax) as amount, concat(c.first_name," ",c.middle_name, c.surname) as contact_person, c.company_name from orders o ';
                $sql .= ' INNER JOIN customers c on c.id=o.customer_id';
                $sql .= ' LEFT JOIN customer_zones as cz ON cz.customer_id=c.id';
                $sql .= ' WHERE o.date BETWEEN "'.$start_week.' 00:00:00" AND "'.$end_week.' 23:59:59"';
                if(isset($postData['zone']))
            ...
            foreach($returnData as $rkey => $return)
            {
                ...
                $sheet->setCellValueByColumnAndRow($col, $row, 'Return Amt');
                $col++;
                $sheet->setCellValueByColumnAndRow($col, $row, 'Zone No');
                $col++;
                $sheet->setCellValueByColumnAndRow($col, $row, 'Route');
                $col++;
                $sheet->setCellValueByColumnAndRow($col, $row, 'Sales Person');
                $col++;
                ...
                $row = 2;
                foreach($customers as $cuKey => $cust)
                {
                    if(isset($customerWiseData[$cust['id']]['pos'])){
                        $pos = $customerWiseData[$cust['id']]['pos'];
                    }else{
                        $pos = 0;
                    }

                    if(isset($customerWiseData[$cust['id']]['app'])){
                        $app = $customerWiseData[$cust['id']]['app'];
                    }else{
                        $app = 0;
                    }
                    if(isset($customerWiseData[$cust['id']]['salesman'])){
                        $sale_by = $customerWiseData[$cust['id']]['salesman'];
                        $salesman = $salePerson[ $sale_by]['person_name'].' / '.$salePerson[ $sale_by]['username'];
                    }else{
                        $salesman = '';
                    }
                    $col = 0;
                    $col++;
                    $sheet->setCellValueByColumnAndRow($col, $row, $cust['name'].'('.$cust['id'].')');
                    $col++;
                    $total_amt = 0;
                    foreach($dateArray as $dKey => $date)
                    {
                        $amt = 0;
                        if(isset($customerWiseData[$cust['id']]['week_count'][$date])){
                            $sheet->setCellValueByColumnAndRow($col, $row, $customerWiseData[$cust['id']]['week_count'][$date]);
                            $amt = $customerWiseData[$cust['id']]['week_count'][$date];
                        }
                        else{
                            $sheet->setCellValueByColumnAndRow($col, $row, 0);
                        }
                    ...
                    $sheet->setCellValueByColumnAndRow($col, $row, $app);
                    $col++;
                    $sheet->setCellValueByColumnAndRow($col, $row, $pos);
                    $col++;
                    if(isset($return_array[$cust['id']]))
                    {
                        $return_nos = $return_array[$cust['id']]['return_nos'];
                        $sum_return_amt = $return_array[$cust['id']]['sum_return_amt'];
                    }
                    else{
                        $return_nos = 0;
                        $sum_return_amt = 0;
                    ...
                    $sheet->setCellValueByColumnAndRow($col, $row, $sum_return_amt);
                    $col++;
                    $sheet->setCellValueByColumnAndRow($col, $row, $cust['zone']);
                    $col++;
                    $sheet->setCellValueByColumnAndRow($col, $row, $cust['route']);
                    $col++;
                    $sheet->setCellValueByColumnAndRow($col, $row, $salesman);
                    $col++;
                    $sheet->setCellValueByColumnAndRow($col, $row, $cust['address']);
                    $row++;
                }
                $writer = new \PhpOffice\PhpSpreadsheet\Writer\Xlsx($spreadsheet);
                ob_end_clean();
        ...
        $data['title'] = "ERP : Orders";
        $data['meta_description'] = "Weekly Sales Report (All Customers)";
        
        $data['modules'][] = "orders";
        $data['methods'][] = "weekly_report_view2";
        $data['js'][] = '<script type="text/javascript">
        $(document).on("change", ".zone",function(){
    ...
    public function weekly_report_view2($data=[])
    {
        $zones =$this->pktdblib->custom_query('SELECT DISTINCT zone_no FROM routes');
        foreach($zones as $eKey=>$zone){
           $data['option']['zone'][$zone['zone_no']] = $zone['zone_no'];
        ...
        $data['salePersons'] = $this->pktdblib->custom_query('select l.id, concat(l.first_name, " ",l.surname) as person_name, l.username from login l INNER JOIN user_roles ur on ur.login_id=l.id where ur.login_id=l.id and ur.role_id=7 AND l.is_active=true');
        $data['formTitle'] = "Weekly Sales Report(All Customers)";
        $this->load->view("orders/weekly_report2", $data);
    ...
    public function area_priority(){
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            $searchQuery = "";
            if($searchValue != ''){
               $searchQuery = " AND (c.city_name like '%".$searchValue."%' or a.area_name like '%".$searchValue."%' or s.state_name')";
            }
            ...
            $sql2 = 'Select * from ('.$sql.') t where 1=1'.$searchQuery.'';
            if ($rowperpage!='-1') {
            ...
            foreach($records as $recordKey => $record){
                ...
                if($record->is_active == 1)
                {
                    ...
                    $action = '<input type="checkbox" class=" update-status" checked  id="'.$record->id.'" name="status">';
                }else{
                    $action = '<input type="checkbox" class=" update-status"  id="'.$record->id.'" name="status">';
                }
        ...
        $data['meta_title'] = "ERP";
        $data['meta_description'] = "Update Zone Priority";
        $data['meta_title'] = 'Update Zone Priority';
        $data['meta_keyword'] = 'Update Zone Priority';
        $data['content'] = 'orders/admin_zone_priority';
        $data['js'][] = '<script type="text/javascript">
        $(document).on("change", ".zone",function(){
    ...
    public function update_area_status()
    {
        ...
        if(!empty($postData)){
            if($postData['status'] == 0)
            {
                ...
                $sql = 'UPDATE areas SET is_active = '.$postData['status'].' WHERE id = '.$postData['id'].'';
            }
            $response = $this->pktdblib->custom_query($sql);
            if(!empty($response))
            {
