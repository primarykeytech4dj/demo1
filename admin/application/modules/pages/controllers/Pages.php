...
class Pages extends MY_Controller {
    ...
    function setup(){
        $setup = $this->page_model->tbl_pages();
        return $setup;
    }
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if($this->form_validation->run()!== FALSE) { //echo "hii";
                ...
                if(!empty($_FILES['featured_image']['name'])) {
                    ...
                    $logoFileValidationParams = ['file'=>$_FILES['featured_image'], 'path'=>'../content/uploads/featured_images/', 'ext'=>'jpg|png|gif|jpeg', 'fieldname'=>'featured_image', 'arrindex'=>'featured_image'];
                    //print_r($logoFileValidationParams);//exit;
                    $featuredImage = $this->pktlib->upload_single_file($logoFileValidationParams);
                ...
                if (empty($error)) {
                    ...
                    //print_r($error['featured_image']);
                    $msg = array('message'=> 'Error while uploading file'.$error['featured_image']['featured_image']['featured_image'], 'class' => 'alert alert-danger');
                        $this->session->set_flashdata('message', $msg);
                }
    	...
    	if($_SERVER['REQUEST_METHOD'] == 'POST') {
    		...
    		if($this->form_validation->run()!== FALSE) {
    			...
    			if(!empty($_FILES['featured_image']['name'])) {
                    ...
                    $logoFileValidationParams = ['file'=>$_FILES['featured_image'], 'path'=>'../content/uploads/featured_images/', 'ext'=>'jpg|png|gif|jpeg', 'fieldname'=>'featured_image', 'arrindex'=>'featured_image'];
                    $profileImg = $this->pktlib->upload_single_file($logoFileValidationParams);
    				if(empty($profileImg['error'])) {
    					...
    					$data['value_posted']['pages']['featured_image'] = $post_data['featured_image2'];
     				}
    			}
    			else {
    				$data['value_posted']['pages']['featured_image'] = $post_data['featured_image'] = $post_data['featured_image2'];
    				unset($post_data['featured_image2']);
    			}
    			/*echo '<pre>';
    			print_r($error);exit;*/
