            ...
            if($this->form_validation->run()!== FALSE) { 
                ...
                if (empty($error)) {
                    ...
                    if($reg_vendor['status'] === 'success') {
                        ...
                        if(NULL!==$post_data['primary_email']){
                            $post_data['account_type'] = 'vendors';
                        ...
                        if(isset($_SESSION['application']['tally']) && $_SESSION['application']['tally']==TRUE){
                            $tallyLedger = Modules::run('tally/import_ledger/', $reg_vendor['id'], 'vendors');
                        }
    		...
    		if($this->form_validation->run('vendors')!== FALSE) {
    			...
    			if(empty($error)) {
    				...
    				if($this->edit_vendor($id, $post_data)) {
                        if(isset($_SESSION['application']['tally']) && $_SESSION['application']['tally']==TRUE){
                            $tallyLedger = Modules::run('tally/import_ledger', $id, 'vendors');
                        }
