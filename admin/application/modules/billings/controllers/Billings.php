...
class Billings extends MY_Controller {
	...
	function bill_listing($data = []) {
		$this->load->model('billings/billing_model');	
		//echo "string"; 
		$this->billing_model->set_table('customer_site_services');
		...
		if($lastdate == $curdate){
			...
			foreach ($query as $key => $customer) {
				...
				foreach ($customerSiteServices as $key => $service) {
					...
					$customerwiseBill['customer_bills']['bill_month'] = date('m', strtotime($lastdate))."-".date('Y');
	...
	function index_courier($id = NULL) {
		...
		$data['js'][] = "<script>
		$(document).on('click', '.payment_received', function(event){
            console.log('hiiii');
            ...
            if (window.confirm('Really Want to update Payment?'))
            {
                //alert('Do you want to Mark Payment');
                window.location = 'update_payment/'+dataId;
            }
            else
            {
                //die();
	...
	function gst_invoice($id = NULL) {
		$data['title'] = $data['meta_title'] = 'Bills';
		$data['meta_description'] = 'Bills';
		$data['modules'][] = 'billings';
		$data['methods'][] = 'gst_courier_bill_listing';
		echo Modules::run("templates/report_template", $data);
	}

	function gst_courier_bill_listing($data = []) {
		$this->load->model('billings/billing_model');	
		//echo "string"; 
		//$this->billing_model->set_table('customer_bills');
		//$bill_month = date("Y-m", strtotime("first day of previous month"));exit;
		if(NULL!==$this->input->post('bill_month')){
			$data['condition']['customer_bills.bill_month'] = $this->input->post('bill_month');
		}else{
			//$data['condition']['customer_bills.bill_month'] = date("m-Y", strtotime("first day of previous month"));
		}
		$data['bills'] = $this->billing_model->get_courier_bills($data);
		/*echo '<pre>';
		print_r($data);exit;*/
		...
		foreach ($data['bills'] as $key => $bill) {
			if($bill['credit_note']>0){
				$data['creditNote'][] = $bill;
			}
		}
		/*echo '<pre>';
		print_r($data['creditNote']);exit;*/
		$this->load->view("billings/gst_courier_bill_listing", $data);
	}
	
	function update_payment($id){
		$this->pktdblib->set_table('customer_bills');
		$bill = $this->pktdblib->get_where($id);
		//$bills = $bill->row_array();
		//echo '<pre>';
		$insertPayment = []; 
		$insertPayment['customer_bill_id'] = $id;
		$insertPayment['amount'] = $bill['amount_after_tax'];
		$insertPayment['is_active'] = true;
		$insertPayment['created'] = $insertPayment['modified'] = date('Y-m-d H:i:s');
		$this->pktdblib->set_table('bill_payments');
		$id = $this->pktdblib->_insert($insertPayment);
		if($id['status'] == 'success'){
			redirect('billings/bills');
		}
		//print_r($update);exit;
		$data['id'] = $id;
		$data['meta_title'] = "Update Payment";
		$data['meta_description'] = "Update Payment";
		$data['content'] = 'billings/update_payment';
		echo Modules::run('templates/admin_template', $data);
		//echo "reached here";echo $id;
	}
}
