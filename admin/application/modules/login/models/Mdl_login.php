...
class Mdl_Login extends CI_Model
{
    ...
    function tbl_setup(){
    	$check = $this->check_table('setup');
    	//print_r($check);exit;
    	if(!$check){
    		//echo "table does not exists<br>";
    		$query = $this->db->query("CREATE TABLE IF NOT EXISTS `setup` (
			  `id` int(11) NOT NULL,
			...
			if($query){
				$insert = $this->db->query("INSERT INTO `setup` (`id`, `parameter`, `value`, `datatype`, `priority`, `created`, `modified`, `is_active`) VALUES
					(1, 'multiple_company', '1', 'checkbox', 1, '".date('Y-m-d H:i:s')."', '".date('Y-m-d H:i:s')."', 1),
					(2, 'share_customer', '0', 'checkbox', 2, '".date('Y-m-d H:i:s')."', '".date('Y-m-d H:i:s')."', 1),
					(3, 'share_employee', '1', 'checkbox', 2, '".date('Y-m-d H:i:s')."', '".date('Y-m-d H:i:s')."', 1),
					(4, 'share_product_categories', '0', 'checkbox', 2, '".date('Y-m-d H:i:s')."', '".date('Y-m-d H:i:s')."', 1),
					(5, 'share_products', '0', 'checkbox', 3, '".date('Y-m-d H:i:s')."', '".date('Y-m-d H:i:s')."', 1),
					(6, 'profile_image', 'default-user.png', '', 1, '".date('Y-m-d H:i:s')."', '".date('Y-m-d H:i:s')."', 1),
					(7, 'default_logo', 'logo.png', 'image', 1, '".date('Y-m-d H:i:s')."', '".date('Y-m-d H:i:s')."', 1),
					(8, 'default_product_image', 'default-product.jpg', 'image', 1, '".date('Y-m-d H:i:s')."', '".date('Y-m-d H:i:s')."', 1),
					(9, 'default-product-category-banner', 'default-banner.jpg', 'image', 1, '".date('Y-m-d H:i:s')."', '".date('Y-m-d H:i:s')."', 1),
					(10, 'default-product-category', 'default.jpg', 'image', 1, '".date('Y-m-d H:i:s')."', '".date('Y-m-d H:i:s')."', 1),
					(11, 'product_has_multiple_brand', '1', 'checkbox', 1, '".date('Y-m-d H:i:s')."', '".date('Y-m-d H:i:s')."', 1);");

    			$check_image_setup = $this->check_table('image_setup');
    			if(!$check_image_setup){
    				$image_setup = $this->db->query("CREATE TABLE IF NOT EXISTS `image_setup` (
					  `id` int(11) NOT NULL,
    			...
    			$check_variation = $this->check_table('variations');
    			if(!$check_variation){
...
(4, 'O+ve', '', 1, '".date('Y-m-d H:i:s')."', '".date('Y-m-d H:i:s')."');");
	...
	function login_listing($orderby = '',$postData) {
	...
	//  echo "<pre>";print_r($postData);exit;
		...
		$searchQuery = "";
		if($searchValue != ''){
		   $searchQuery = " AND (t.account_type like '%".$searchValue."%' or t.first_name like '%".$searchValue."%' or t.username like'%".$searchValue."%' or t.surname like'%".$searchValue."%' or t.email like '%".$searchValue."%')";
		}
     	$this->db->select(['login.*', '(select GROUP_CONCAT(roles.role_name SEPARATOR ", ") from roles inner join user_roles on user_roles.role_id=roles.id where user_roles.login_id=login.id) as roles']);
		 $this->db->order_by($orderby);
     	// $db->join('user_roles', 'user_roles');

		 $sql = $this->db->get_compiled_select('login');
