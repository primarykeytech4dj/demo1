...
class Products extends MY_Controller {
	...
	function get_single_product($slug = ''){
		...
		$conditions2 = array( 
            'start' => 0, 
            'limit' => $this->perPage,
            'condition' => ['productslug'=>$slug]
        ); 

        $product = $this->product_model->_get_all_products($conditions2);
        $product = $product[0];
        $product['product_attribute_id'] = 0;
		...
		$data['product']['attributes'] = $this->product_attributes($product['id']);
        ...
        $data['product']['productVariation'] = [];
		foreach ($variations as $itemKey => $itemValue) {
			$data['product']['productVariation'][$itemValue['name']][]= $itemValue['value'];
		}
		...
		// echo 'Select * from pack_products where basket_id="'.$product['id'].'" and is_active=true';exit;
		$data['product']['packProducts'] = $this->getPackProducts($product['id']);
		
		$data['product']['productImages'] = $this->get_productwise_images($product['id']);
		//echo "hii";exit;
		//print_r($product);exit;
		...
		$conditions2 = array( 
            'start' => 0, 
            'limit' => $this->perPage,
            'condition' => ['product_category_id'=>$product['product_category_id']]
        ); 
        
        /*$data['relatedProducts'] = $this->product_model->_get_all_products($conditions2);
        foreach($data['relatedProducts'] as $key=>$rproduct){
    	    $rattribute = $this->product_default_attribute($rproduct['id']);
    	    if($rattribute){
    	        foreach($rattribute as $attKey=>$att)
    	        {
    	            $data['relatedProducts'][$key][$attKey] = $att;
    	        }
    	    }
    	    
    	    $data['relatedProducts'][$key]['attributes'] = $this->product_attributes($rproduct['id']);
    	}*/
        //echo $this->db->last_query();exit;
        //$this->get_related_products($category['id'], $product['id']);
	...
	function featured_product($data = []) {
    	...
    	if(isset($data['order']))
    		...
    		$sql.=" ORDER BY products.priority ASC, products.modified DESC";
...
INNER JOIN Shippers ON Orders.ShipperID = Shippers.ShipperID)";*/
    ...
    function all_product($page=NULL) {
        ...
        foreach($data['allProduct'] as $key=>$product){
            //$data['allProduct'][$key]['actual_price'] = number_format($product['actual_price'], 2);
    	    $attribute = $this->product_default_attribute($product['id']);
    	    if($attribute){
    	        foreach($attribute as $attKey=>$att)
    	        {
	...
	function product_default_attribute($productId, $productAttributeId=''){
	    //echo "hii -".$productId." -".$productAttributeId."<br>";
	    $product = $this->pktdblib->custom_query('Select * from products where id='.$productId);
	    $defaultAttr = [];
	    $sql = 'Select p.base_price, p.base_uom, pa.*, concat(a.unit, " ",a.uom) as uom2 from products p inner join product_attributes pa on pa.product_id=p.id inner join attributes a on a.id=pa.attribute_id where pa.product_id="'.$productId.'" and pa.is_active=true';
	    if(!$product[0]['overall_stock_mgmt']){
	        
	        $sql.= ' AND pa.stock_qty>0';
	    }
	    //echo $productAttributeId;exit;
	    if($productAttributeId!=''){
	        $sql.=' AND pa.id='.$productAttributeId;
	    }else{
	        $sql.='  and pa.is_default=1';
	    }
	    
	    $sql.=' order by pa.id asc limit 1';
	    //echo $sql;//exit;
	    $defaultAttr = $this->pktdblib->custom_query($sql);
	    //print_r($defaultAttr);exit;
	    if(empty($defaultAttr)){
	         $defaultAttr = $this->pktdblib->custom_query('Select p.base_price, p.base_uom,pa.*, concat(a.unit, " ",a.uom) as uom2 from products p inner join product_attributes pa on pa.product_id=p.id inner join attributes a on a.id=pa.attribute_id where pa.product_id="'.$productId.'" and pa.is_active=true AND pa.stock_qty>0 order by pa.id asc limit 1');
	    }
	    //print_r($defaultAttr);//exit;
	    if(!empty($defaultAttr)){
	       ...
	       $response = false;
	       //if($priceList){
	       $actualPrice = $defaultAttr[0]['price'];//$defaultAttr[0]['mrp']-(($defaultAttr[0]['discount']/100.00)*$defaultAttr[0]['mrp']);
	       $actualPrice = number_format( (float)$actualPrice , (int) $decimals = 0 , (string) $dec_point = '.' , (string) $thousands_sep = '' );
	           $response = ['product_attribute_id'=>$defaultAttr[0]['id'], 'discount'=>$defaultAttr[0]['discount'], 'default_uom'=>$defaultAttr[0]['uom2'], 'actual_price'=>$actualPrice, 'mrp'=>$defaultAttr[0]['mrp']];
	       //}
	...
	function product_attributes($productId){
	    ...
	    $productAttr = [];
	    if($product[0]['overall_stock_mgmt']){
	        $productAttr = $this->pktdblib->custom_query('Select p.base_price, p.base_uom, pa.id, pa.attribute_id, a.uom,pa.increased_percentage,pa.mrp,pa.discount,pa.price, concat(a.unit, " ",a.uom) as uom2, a.unit from products p inner join product_attributes pa on pa.product_id=p.id inner join attributes a on a.id=pa.attribute_id where pa.product_id="'.$productId.'" and pa.is_active=true order by pa.id asc');
	    }else{
	        $productAttr = $this->pktdblib->custom_query('Select p.base_price, p.base_uom, pa.id, pa.attribute_id, a.uom,pa.increased_percentage,pa.mrp,pa.discount,pa.price, concat(a.unit, " ",a.uom) as uom2, a.unit from products p inner join product_attributes pa on pa.product_id=p.id inner join attributes a on a.id=pa.attribute_id where pa.product_id="'.$productId.'" and pa.is_active=true AND pa.stock_qty>0 order by pa.id asc');
	    }
	    ...
	    foreach($productAttr as $attKey=>$att){
	       ...
	       //if($priceList){
	           $actualPrice = $att['price'];//$att['mrp']-(($att['discount']/100.00)*$att['mrp']);
	           $actualPrice = number_format( (float)$actualPrice , (int) $decimals = 0 , (string) $dec_point = '.' , (string) $thousands_sep = '' );
	           $response[] = ['product_attribute_id'=>$att['id'], 'discount'=>$att['discount'], 'default_uom'=>$att['uom2'], 'actual_price'=>$actualPrice, 'mrp'=>$att['mrp'], 'unit'=>$att['unit'], 'uom'=>$att['uom'], 'display_unit'=>$att['uom2']];
	       //}
	    }
	    //$this->pktlib->parseOutput($this->config->item('response_format'), $response);
	...
	function getPackProducts($productId=NULL){
	    if(NULL===$productId)
	        return [];
	    $basketProducts = [];
	    $packProducts = $this->pktdblib->custom_query('Select * from pack_products where basket_id="'.$productId.'"');
	    //$this->pktdblib->set_table('products');
	    foreach($packProducts as $key=>$pack){
	        
	        $basket = $this->pktdblib->custom_query('Select p.product, i.image_name_1, c.category_name from products p inner join product_images i on i.product_id=p.id and i.featured_image=true inner join product_categories c on c.id=p.product_category_id where p.id='.$pack['product_id']);
	        $basketProducts[$key] = $basket[0];
	        if($pack['product_attribute_id']!=0){ //echo "hii";
	            $attribute = $this->product_default_attribute($pack['product_id'], $pack['product_attribute_id']);
	            /*print_r($attribute);
	            exit;*/
	            $basketProducts[$key] = array_merge($basketProducts[$key], $this->product_default_attribute($pack['product_id'], $pack['product_attribute_id']));
	        }
	        //print_r($basketProducts[$key]);
	    }
	    //print_r($basketProducts);exit;
