	...
	function add()
	{
        ...
        if($_SERVER['REQUEST_METHOD']=='POST'){
            ...
            if($this->form_validation->run()!==FALSE){
                ...
                if(count($chk)>0){
                    $del = $chk = $this->pktdblib->custom_query('delete from wishlist where user_id="'.$postData['user_id'].'" and product_id="'.$postData['wishlist']['product_id'].'"');
                    if($del){
                        if($this->input->is_ajax_request() || !empty($params)){
                            echo json_encode(['status'=>'success', 'msg'=>'Product removed from wishlist', 'is_added_to_wishlist'=>false]);
                            exit;
                        }else{
                            $msg = array('message' => "Product removed wishlist", 'class' => 'alert alert-success', 'is_added_to_wishlist'=>false);
                    ...
                    if($id['status']='success'){
                        if($this->input->is_ajax_request() || !empty($params)){
                            echo json_encode(['status'=>'success', 'msg'=>'Product added to wishlist', 'is_added_to_wishlist'=>true]);
                            exit;
                        }else{
                            $msg = array('message' => "Product added to wishlist", 'class' => 'alert alert-success', 'is_added_to_wishlist'=>true);
               		        $this->session->set_flashdata('message',$msg);
                        }
                    }
                    else{
                        if($this->input->is_ajax_request() || !empty($params)){
                            echo json_encode(['status'=>'failed', 'msg'=>'Failed to add in wishlist', 'is_added_to_wishlist'=>false]);
                            exit;
                        }else{
                            $msg = array('message' => "Failed to add in wishlist", 'class' => 'alert alert-danger', 'is_added_to_wishlist'=>false);
               		        $this->session->set_flashdata('message',$msg);
                        }
                    }
			    ...
			    if($this->input->is_ajax_request() || !empty($params)){
                    ...
                    $msg = array('message' => "Following Validation Error Occurred.".validation_errors(), 'class' => 'alert alert-danger', 'is_added_to_wishlist'=>false);
               		$this->session->set_flashdata('message',$msg);
                }
			}
        }
