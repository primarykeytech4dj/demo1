...
//db->join($table.' c2', 'c2.parent_id='.$table.'.id', 'left');
    ...
    function _get_all_products($params = array()){
        $this->db->select(["products.*", "product_images.image_name_1", "product_images.image_name_2", "product_details.discount_type", "product_details.discount", "(CASE WHEN product_details.in_stock_qty=NULL THEN 0.00 ELSE product_details.in_stock_qty END) as in_stock_qty" , "(CASE WHEN product_details.discount_type='percentage' THEN products.base_price+(products.base_price*(product_details.discount/100)) WHEN product_details.discount_type='value' THEN products.base_price+product_details.discount ELSE products.base_price END) as actual_price"]);
        ...
        if(isset($params['condition'])){
            ...
            if(isset($params['condition']['product_category_id']) && $params['condition']['product_category_id']!=0){
                $this->db->where(['products.product_category_id'=>$params['condition']['product_category_id']]);
            }
            
            if(isset($params['condition']['brand']) && !empty(trim($params['condition']['brand']))){
