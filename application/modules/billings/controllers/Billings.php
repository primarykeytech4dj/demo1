...
class Billings extends MY_Controller {
	...
	function cron_courier_billing_end_of_month(){
	    ...
	    $previousMonth = date('Y-m-d', strtotime('last day of previous month'));
		$firstdate = date('Y-m-d', strtotime('first day of previous month'));;//'2018-07-01';//date('Y-m-01');
		$lastdate = date('Y-m-d', strtotime('last day of previous month'));//date('Y-m-t',strtotime('today'));
		//$curdate = '2018-07-31';//date('Y-m-d');
		//if($lastdate == $curdate){
		    //echo "reached inside";exit;
			$billingData = [];
			//echo '<pre>';
			...
			foreach ($query as $key => $customer) {
                ...
                if(empty($customerwiseBill['customer_bills']))
                    continue;
				...
				if($bill['status']=='success'){
					...
					if($update){
						...
						$mail = Modules::run('billings/mail_courier_invoice', base64_encode($customerwiseBill['customer_bills']['invoice_no']));
					}
				}
			}
	...
	function mail_courier_invoice($invoiceId){
	    $this->pktdblib->set_table('customer_bills');
		$query = $this->pktdblib->get_where_custom('invoice_no', base64_decode($invoiceId));
		...
		$html = Modules::run('billings/load_courier_bill_pdf', $invoiceId);
		//echo $html;exit;
		...
		$this->email->subject("YOUR COURIER BILL HAS BEEN GENERATED FROM ".date('d F, y', strtotime($bill['bill_from_date'])). ' TO '.date('d F, y', strtotime($bill['bill_to_date'])));
	...
	function load_courier_bill_pdf($invoiceId = NULL){
	    if(NULL==$invoiceId){
	        show_404();
	        exit;
	    }
		...
		$data['customer_bill_details'] = $this->pktdblib->custom_query('select * from customer_bill_details where customer_bill_id='.$data['customer_bills']['id'].' order by date asc');
		/*echo '<pre>';
