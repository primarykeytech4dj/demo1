...
class Pages extends MY_Controller {
    ...
    function view($slug) {
        //print_r($slug);
        $this->page_model->set_table('pages');
        $pages = $this->page_model->get_where_custom('slug', $this->uri->segment(3));
        //print_r($pages);
        $data['pages'] = $pages->row_array();
        $data['meta_title'] = $data['pages']['title'];
        $data['meta_keyword'] = $data['pages']['title'];
        $data['title'] = $data['pages']['title'];
        $data['meta_description'] = $data['pages']['title'];
        ...
        if($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if($this->form_validation->run()!== FALSE) { //echo "hii";
                ...
                if(!empty($_FILES['featured_image']['name'])) {
                    $profileFileValidationParams = ['file' => $_FILES['featured_image'], 'path'=> './assets/uploads/featured_images/', 'ext' => 'gif|jpg|png|jpeg', 'fieldname' =>'featured_image', 'arrindex' =>'featured_image'];
	...
	function admin_add_page($data = []) {
        /*$categories = $this->page_model->_get_distinct_category();
        print_r($categories);
        foreach ($categories as $key => $category) {
            $data['categories'][$category] = $category;
        }*/
		$this->load->view("pages/admin_add", $data);
		//$this->load->view("admin_panel/login_register");
	}
