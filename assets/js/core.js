function init(){
  ...
  if(!$(".example2").dataTable.isDataTable()){
    //alert("hello");
    $('.example2').DataTable({
      "paging": true,
      "lengthChange": true,
...
// Add more for colleges aluminis
    ...
    $('.AddMoreRow').on('click', function(){
        ...
        trlast.find('input, select, textarea, checkbox').each(function() {

          if($(this).hasClass('filter')){
            var datatarget = $(this).attr('data-target');
            var tgt = datatarget.split('_');
            tgt[tgt.length-1] = newId;
            
            datatarget = tgt.join('_');
    ...
    event.preventDefault();
        ...
        if (typeof dataId !== typeof undefined && dataId !== false) {
          ...
          $.ajax({
            ...
            dataType: 'json',
            data: {'id':dataId, 'is_active':dataValue, 'table':datatable},
  ...
  $(document).on('onkeyup change blur', '.calculate', function(){
    //alert(this.id);
    var id = this.id;
    var trId = $("#"+id).closest('tr').attr('id');
    var amt = $("#unit_price_"+trId).val()*$("#qty_"+trId).val();
    var tax = $("#unit_price_"+trId).val();
        ...
        success: function (data) {
            ...
            if(data.status==0){
              ...
              if(typeof modalId != 'undefined'){
                  if(typeof $("#"+modalId).attr('data-refill-target')!='undefined'){
...
$(document).on('change click', '.filter', function(event) {
        ...
        if(value!='na'){
          ...
          $('#'+formId+' #'+datatarget).select2('destroy').empty().select2({data : [{id:0, text:'NA'}]});
        }
    });
    ...
    $('.sitetab').on('click', function(){
        ...
        $('body #'+target).addClass('active').html('<center>Loading...</center>');
        //console.log(target+ " "+url);
        $.ajax({
            url : base_url+''+url,
            success: function(response) {
                //alert(response);
            $('body #'+target).html(response);
            init();
        }
    });
});
