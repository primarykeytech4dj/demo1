...
(function(factory) {
    ...
    OrgChart.prototype = {
        ...
        createNode: function(nodeData, level, opts) {
            ...
            var $nodeDiv = $('<div' + (opts.draggable ? ' draggable="true"' : '') + (nodeData[opts.nodeId] ? ' id="' + nodeData[opts.nodeId] + '"' : '') + (nodeData.parentId ? ' data-parent="' + nodeData.parentId + '"' : '') + (nodeData.id ? ' data-id="' + nodeData.id + '"' : '') + (nodeData.parent_id ? ' data-parent_id="' + nodeData.parent_id + '"' : '') + '>')
            ...
            $nodeDiv.on('mouseenter mouseleave', function(event) {
                ...
                if (event.type === 'mouseenter') {
                    ...
                    $.ajax({
                        url: base_url+'mlm_reports/ajax_hierarchy',
                        data: {
                            cust_id: $(this).attr('data-id')
                        },
                        success: function(resp) {
                            $('#' + this_id).append('<div class="appended_response">'+resp+'</div>');
                        }
                    });
            ...
            $nodeDiv.on('click', '.bottomEdge', function(event) {
                ...
                if (childrenState.exist) {
                    ...
                    if ($children.find('.node:visible').is('.sliding')) {
                        return;
                    }
